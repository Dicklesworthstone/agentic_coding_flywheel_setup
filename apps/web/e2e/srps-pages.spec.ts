  ...
  .serial("SRPS Website Pages", () => {
      ...
      test("SRPS lesson loads without JS errors", async ({ page }) => {
        ...
        await page.evaluate(() => {
          localStorage.setItem(
            ...
            JSON.stringify(Array.from({ length: 23 }, (_, i) => i)),
          );
        });
    ...
    test.describe("SRPS on Flywheel Page", () => {
      test("SRPS appears in flywheel stack", async ({ page }) => {
        ...
        await expect(
          page
            .getByText(/\bSRPS\b|System Resource/i)
            .filter({ visible: true })
            .first(),
        ).toBeVisible({ timeout: 5000 });
      });

      test("flywheel page mentions System Resource Protection", async ({ page }) => {
        // Unlock lesson 23 (SRPS)
        await page.goto("/");
        await page.evaluate(() => {
          localStorage.setItem(
        ...
        const description = page.getByText(/resource protection|responsive|ananicy/i).first();
    ...
    test.describe("SRPS on TLDR Page", () => {
      ...
      test("TLDR page loads without JS errors with SRPS entry", async ({ page }) => {
        const errors: string[] = [];
      ...
      test("can navigate from learn to SRPS lesson page", async ({ page }) => {
        ...
        await page.addInitScript(() => {
          window.localStorage.setItem(
            "acfs-learning-hub-completed-lessons",
            JSON.stringify(Array.from({ length: 50 }, (_, i) => i)),
          );
        });
