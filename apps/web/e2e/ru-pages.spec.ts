  ...
  .serial("RU Website Pages", () => {
    ...
    test.describe("RU Lesson Page", () => {
      test("RU lesson loads without JS errors", async ({ page }) => {
        ...
        await page.evaluate(() => {
          localStorage.setItem(
            ...
            JSON.stringify(Array.from({ length: 20 }, (_, i) => i)),
          );
        });
      ...
      test("RU lesson shows all major sections", async ({ page }) => {
        ...
        await expect(page.getByText(/sync|status|agent.*sweep/i).first()).toBeVisible();
      });
    });
    ...
    test.describe("RU Synergies", () => {
      test("RU synergies with other tools visible", async ({ page }) => {
        ...
        const synergyText = page.getByText(/multi.*repo.*orchestra|repo.*coordination/i);
