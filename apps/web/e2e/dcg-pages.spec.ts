  ...
  .serial("DCG Website Pages", () => {
      ...
      test("DCG lesson loads without JS errors", async ({ page }) => {
        ...
        await page.evaluate(() => {
          localStorage.setItem(
            ...
            JSON.stringify(Array.from({ length: 21 }, (_, i) => i)),
          );
        });
    ...
    test.describe("DCG Glossary Entry", () => {
      ...
      test("DCG glossary entry expands definition", async ({ page }) => {
        ...
        await expect(dcgCard.getByText(/Destructive Command Guard/i).first()).toBeVisible();
      });

      test("glossary page loads without JS errors with DCG entry", async ({ page }) => {
        const errors: string[] = [];
