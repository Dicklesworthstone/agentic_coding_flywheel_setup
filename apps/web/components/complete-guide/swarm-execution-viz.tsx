...
import { AlertTriangle, CheckCircle2, Play, RotateCcw, Users } from "lucide-react";
...
export function SwarmExecutionViz() {
  ...
  const [beadClaims, setBeadClaims] = useState<(number | "collision" | null)[]>(
    Array(BEAD_COUNT).fill(null),
  );
  ...
  return (
    <div
      ref={ref}
      className="my-10 rounded-2xl border border-white/[0.08] bg-[#0a0a0c] shadow-2xl glass-subtle overflow-hidden"
    >
      {/* HEADER */}
        ...
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h4 className="text-sm font-semibold text-white/90">
              Interactive: Staggered Starts vs. Thundering Herd
            </h4>
          </div>
          <p className="text-xs text-white/50">
            Watch how agent initialization timing affects lock contention.
          </p>
        </div>
      ...
      <div className="p-6 relative bg-[radial-gradient(ellipse_at_top,rgba(16,185,129,0.05),transparent_60%)]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-4">
            <h5 className="text-xs font-mono text-white/40 mb-2 uppercase tracking-wider">
              Agent Swarm
            </h5>
            {Array(AGENT_COUNT)
              .fill(0)
              .map((_, i) => (
                <div
                  key={`agent-${i}`}
                  className="relative bg-black/40 border border-white/5 rounded-xl p-3 flex items-center gap-4 overflow-hidden"
                >
                  <div
                    className="absolute top-0 left-0 bottom-0 bg-white/[0.02]"
                    style={{ width: `${agentProgress[i]}%`, transition: "width 0.3s ease-out" }}
                  />
          ...
          <div>
            ...
            <div className="flex items-center gap-3 text-xs text-white/60">
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full border border-white/20 bg-white/5" />{" "}
                Unclaimed
              </div>
              <div className="flex items-center gap-1.5">
                <span className="w-2 h-2 rounded-full bg-[#FF5500] shadow-[0_0_8px_rgba(255,85,0,0.5)]" />{" "}
                In Progress
              </div>
              <div className="flex items-center gap-1.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              {beadClaims.map((claim, i) => {
                ...
                return (
                  ...
                  >
                    {isCollision && <div className="absolute inset-0 bg-red-500/5 animate-pulse" />}
