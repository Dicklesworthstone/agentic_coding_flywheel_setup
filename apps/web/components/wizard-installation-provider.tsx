...
import {
  type ApprovedTeamProfileInstallation,
  activateTeamProfileInstallation,
  discardTeamProfileInstallation,
  readTeamProfileSessionGuard,
  type TeamProfileFileReview,
  TeamProfileImportError,
  type TeamProfileReviewContext,
  teamProfileInstallationMatches,
...
import {
  WizardInstallationContext,
  type WizardInstallationSession,
} from "@/lib/wizardInstallation";
...
function guard(): Guard {
  try {
    return readTeamProfileSessionGuard(window.sessionStorage);
  } catch {
    return "unavailable";
  }
}
...
function currentContext(reviewed: TeamProfileReviewContext): TeamProfileReviewContext {
  return {
    targetHost: getVPSIP() ?? "",
    current: {
      providerSelection: getVPSReadinessSelection(),
      installMode: getInstallMode(),
      ref: getACFSRef(),
      username: getSSHUsername(),
      architecture: reviewed.current.architecture,
      ubuntuVersion: reviewed.current.ubuntuVersion,
...
export function WizardInstallationProvider({ children }: { children: ReactNode }) {
  ...
  const inWizard =
    pathname === null || pathname === "/wizard" || pathname?.startsWith("/wizard/") === true;
  ...
  if (active && ready && storedGuard === "review_required") {
    try {
      ...
      activeMatches =
        teamProfileInstallationMatches(value, currentContext(active.context)) &&
        buildInstallCommand(value.mode, value.ref, value.username, value.moduleSelection) ===
          value.command;
    } catch {
      activeMatches = false;
    }
  ...
  const status: WizardInstallationSession["status"] =
    pathname === null || !hydrated || !ready
      ? "loading"
      : storedGuard === "unavailable"
        ? "unavailable"
        : active && activeMatches
          ? "active"
          : reviewRequired || active !== null || storedGuard === "review_required"
            ? "review_required"
            : "saved";
    ...
    (review: TeamProfileFileReview, reviewed: TeamProfileReviewContext, confirmed: boolean) => {
      if (!mounted.current || !ready || !inWizard || pathname === null) {
        throw new TeamProfileImportError(
          ...
          "Wait for the current wizard settings before selecting an installation.",
        );
      }
      ...
      if (!teamProfileReviewMatches(review, current)) {
        throw new TeamProfileImportError(
          ...
          "Saved settings or the target changed. Review the profile again.",
        );
      }
      try {
        const value = activateTeamProfileInstallation(
          review,
          current,
          confirmed,
        ...
        if (
          ...
          throw new TeamProfileImportError(
            "team_profile_review_changed",
            "The installation command changed. Review the profile again.",
          );
        }
  ...
  const discard = useCallback(() => {
    ...
    try {
      ...
      setMessage(
        "The pending-profile guard could not be cleared. No saved/default installation has been selected.",
      );
    }
  }, []);
  const blockEdit = useCallback(() => {
    setMessage(
      "Discard the reviewed installation explicitly before editing individual saved settings. Its approved choices have not changed.",
    );
  }, []);
  const session = useMemo<WizardInstallationSession>(
  ...
  return (
    <WizardInstallationContext.Provider value={session}>
      {status === "loading" ? (
        <main id="main-content" className="mx-auto max-w-2xl p-8" role="status">
          ...
          <section role="alert" className="space-y-3 rounded-lg border border-destructive/40 p-4">
            ...
            <p>
              {status === "unavailable"
                ...
                : "This tab has a pending or changed team installation. Reloading never restores approval or silently selects the default installation."}
            </p>
            <p>
              Review the file again below, or explicitly discard it to return to your saved wizard
              settings.
            </p>
            <Button type="button" variant="outline" onClick={discard}>
              Discard reviewed installation and use saved settings
            </Button>
            {message && <p>{message}</p>}
          </section>
        ...
        <>
          {installation && (
            <aside
              ...
              aria-label="Active reviewed installation"
            >
              <p className="font-semibold">
                Using reviewed team profile: {installation.displayName}
              </p>
              <p className="text-sm">
                The exact reviewed mode, ref, username and module selections apply throughout this
                wizard tab. Saved preferences are unchanged.
              </p>
              <Button type="button" variant="outline" onClick={discard}>
                Discard reviewed installation and use saved settings
              </Button>
