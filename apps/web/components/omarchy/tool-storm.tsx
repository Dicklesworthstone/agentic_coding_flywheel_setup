 ...
 *
 * Guards: DPR ≤ 1.5 (adaptive down-grade if FPS sags), RAF paused while
 * document.hidden, prefers-reduced-motion renders a single composed frame,
...
// Tools rendered as glowing name plates riding the disk
const TOOL_NAMES = [
...
export default function ToolStorm({ className }: { className?: string }) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL — leave the CSS gradient background as the fallback
      return;
    }
    ...
    const applyFrame = (dt: number) => {
      ...
      for (const rider of riders) {
        ...
        const target =
          (inTextZone ? 0.1 : 1) * depthDim * rider.baseOpacity;
