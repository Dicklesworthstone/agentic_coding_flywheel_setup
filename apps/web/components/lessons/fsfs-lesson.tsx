    ...
    <div className="space-y-8">
      ...
      <Section title="What Is FrankenSearch?" icon={<Search className="h-5 w-5" />} delay={0.1}>
        <Paragraph>
          ...
          combining lexical (BM25) and semantic retrieval with progressive delivery. It indexes your
          local files and returns results ranked by both keyword relevance and meaning.
        </Paragraph>
        <Paragraph>
          FSFS builds an index once, then serves queries in milliseconds. It supports incremental
          updates so new files are searchable without a full re-index.
        </Paragraph>
      ...
      <Section title="Essential Commands" icon={<Terminal className="h-5 w-5" />} delay={0.2}>
        ...
        <TipBox>
          Use FSFS when you need to search across large codebases or documentation sets where simple
          grep isn&apos;t enough.
        </TipBox>
      </Section>
      ...
      <Section
        title="Try It: Hybrid Search Laboratory"
        icon={<Search className="h-5 w-5" />}
        delay={0.25}
      >
        <Paragraph>
...
const SCENARIOS: QueryScenario[] = [
  {
    ...
    results: [
      {
        ...
        cosineScore: 0.72,
        fusedScore: 0.96,
        keywordMatches: ["retryWithBackoff"],
        semanticLinks: ["retry logic"],
      },
      {
        id: "e2",
        ...
        fusedScore: 0.82,
        keywordMatches: ["retryWithBackoff"],
        semanticLinks: ["HTTP retry"],
      },
      {
        id: "e3",
        title: "CircuitBreaker",
        file: "src/resilience/circuit.ts",
        snippet: "Falls back to retryWithBackoff when circuit is half-open",
        bm25Score: 0.61,
        cosineScore: 0.79,
        fusedScore: 0.73,
        keywordMatches: ["retryWithBackoff"],
        semanticLinks: ["fault tolerance"],
      },
    ],
    bm25TimeMs: 8,
    vectorTimeMs: 310,
    fusionTimeMs: 4,
    insight:
      "BM25 finds the exact token instantly. Semantic search still adds the CircuitBreaker result which is conceptually related.",
  ...
  },
  {
    ...
    results: [
      {
        ...
        cosineScore: 0.95,
        fusedScore: 0.88,
        keywordMatches: ["failures"],
        semanticLinks: ["cascading failure prevention", "circuit breaker"],
      },
      {
        id: "s2",
        title: "BulkheadIsolation",
        ...
        fusedScore: 0.78,
        keywordMatches: [],
        semanticLinks: ["fault isolation", "resilience"],
      },
      {
        id: "s3",
        title: "retryWithBackoff()",
        ...
        fusedScore: 0.7,
        keywordMatches: ["failures"],
        semanticLinks: ["transient fault handling"],
      },
      {
        id: "s4",
        title: "HealthCheck",
        ...
        fusedScore: 0.58,
        keywordMatches: [],
        semanticLinks: ["system health", "degradation detection"],
      },
    ],
    bm25TimeMs: 6,
    vectorTimeMs: 380,
    fusionTimeMs: 5,
    insight:
      "BM25 barely matches (no exact terms in code). Semantic search finds 4 highly relevant results by understanding the intent.",
  ...
  },
  {
    ...
    description:
      "Misspelled query -- BM25 fails on typos but semantic embeddings handle fuzzy meaning.",
    tokens: [
    ...
    results: [
      {
        ...
        fusedScore: 0.8,
        keywordMatches: [],
        semanticLinks: ["authentication", "middleware pattern"],
      },
      {
        id: "t2",
        title: "sessionValidator()",
        ...
        fusedScore: 0.68,
        keywordMatches: [],
        semanticLinks: ["session auth", "JWT validation"],
      },
      {
        id: "t3",
        title: "requireAuth",
        ...
        fusedScore: 0.63,
        keywordMatches: [],
        semanticLinks: ["role-based access", "auth decorator"],
      },
    ],
    bm25TimeMs: 5,
    vectorTimeMs: 295,
    fusionTimeMs: 3,
    insight:
      "BM25 scores near zero because the typos mismatch exact tokens. Semantic search recovers perfectly because the embedding captures intent.",
  ...
  },
  {
    ...
    description:
      "Searching for a code pattern -- BM25 matches syntax tokens, semantics finds generators.",
    tokens: [
    ...
    results: [
      {
        id: "c1",
        title: "streamResults()",
        file: "src/search/streamer.ts",
        snippet:
          "async function* streamResults(query) { for await (const batch of ...) yield batch; }",
        bm25Score: 0.93,
        cosineScore: 0.88,
        fusedScore: 0.95,
        keywordMatches: ["async", "function*", "yield"],
        semanticLinks: ["async generator", "streaming"],
      },
      {
        id: "c2",
        title: "paginateQuery()",
        file: "src/db/paginator.ts",
        snippet: "async function* paginateQuery(sql, pageSize = 100) { yield* chunks; }",
        bm25Score: 0.87,
        cosineScore: 0.82,
        fusedScore: 0.89,
        keywordMatches: ["async", "function*", "yield"],
        semanticLinks: ["pagination", "lazy evaluation"],
      },
      {
        id: "c3",
        title: "EventEmitter",
        ...
        cosineScore: 0.85,
        fusedScore: 0.74,
        keywordMatches: ["yield"],
        semanticLinks: ["async iteration", "event stream"],
      },
    ],
    bm25TimeMs: 11,
    vectorTimeMs: 340,
    fusionTimeMs: 4,
    insight:
      "Both engines contribute strongly. BM25 catches exact syntax matches while semantic search finds the conceptually related EventEmitter.",
  ...
  },
  {
    ...
    description:
      'German query for "error handling retry" -- BM25 cannot match, semantic embeddings bridge languages.',
    tokens: [
    ...
    results: [
      {
        ...
        fusedScore: 0.68,
        keywordMatches: [],
        semanticLinks: ["retry = Wiederholung", "error handling"],
      },
      {
        id: "m2",
        title: "ErrorBoundary",
        ...
        fusedScore: 0.63,
        keywordMatches: [],
        semanticLinks: ["Fehlerbehandlung = error handling"],
      },
      {
        id: "m3",
        title: "gracefulDegradation()",
        file: "src/utils/fallback.ts",
        snippet: "Returns cached result when upstream service is unavailable",
        bm25Score: 0.0,
        cosineScore: 0.77,
        fusedScore: 0.56,
        keywordMatches: [],
        semanticLinks: ["fault tolerance", "graceful fallback"],
      },
    ],
    bm25TimeMs: 3,
    vectorTimeMs: 420,
    fusionTimeMs: 3,
    insight:
      "BM25 returns zero results (no German tokens in codebase). Semantic search bridges the language gap via multilingual embeddings.",
  ...
  },
  {
    ...
    description:
      "Classic hybrid query -- watch how fusion re-ranks results from both engines for optimal ordering.",
    tokens: [
    ...
    results: [
      {
        ...
        cosineScore: 0.88,
        fusedScore: 0.96,
        keywordMatches: ["database", "connection", "pool"],
        semanticLinks: ["connection pooling"],
      },
      {
        id: "r2",
        ...
        cosineScore: 0.92,
        fusedScore: 0.89,
        keywordMatches: ["database", "connection"],
        semanticLinks: ["connection lifecycle", "resource management"],
      },
      {
        id: "r3",
        ...
        cosineScore: 0.65,
        fusedScore: 0.83,
        keywordMatches: ["database", "pool"],
        semanticLinks: ["configuration"],
      },
      {
        id: "r4",
        ...
        fusedScore: 0.6,
        keywordMatches: ["pool"],
        semanticLinks: ["schema migration", "pooled connections"],
      },
    ],
    bm25TimeMs: 10,
    vectorTimeMs: 410,
    fusionTimeMs: 6,
    insight:
      "ConnectionManager jumps from #3 (BM25) to #2 (fused) because its high semantic score boosts it above DatabaseConfig.",
          ...
          >
            <div
              className={`h-2 rounded-full transition-[width,background-color] duration-300 ${
                idx === activeIndex ? "w-6 bg-purple-400" : "w-2 bg-white/20 hover:bg-white/40"
              }`}
            />
            {idx === activeIndex && (
              <motion.div
        ...
        return (
          <div key={s.key} className="flex items-center gap-1">
            ...
            >
              ...
              {isActive && <Loader2 className="h-2.5 w-2.5 text-purple-400 animate-spin" />}
            </motion.div>
    ...
    >
      ...
      <AnimatePresence>
        {showScores && (
          ...
          >
            <ScoreBar
              value={result.bm25Score}
              color="bg-gradient-to-r from-purple-500/60 to-purple-400/40"
              label="BM25"
              delay={index * 0.04}
            />
            <ScoreBar
              value={result.cosineScore}
              color="bg-gradient-to-r from-cyan-500/60 to-cyan-400/40"
              label="Cos"
              delay={index * 0.04 + 0.05}
            />
            <ScoreBar
              value={result.fusedScore}
              color="bg-gradient-to-r from-amber-500/60 to-amber-400/40"
              label="Fused"
...
function RelevanceGauge({
  ...
  const improvement =
    fusedAvg > 0
      ? Math.round(
          ((fusedAvg - Math.max(bm25Avg, cosineAvg)) / Math.max(bm25Avg, cosineAvg, 0.01)) * 100,
        )
      : 0;
  const arcRadius = 42;
                  ...
                  <div className="rounded-xl border border-amber-500/20 bg-amber-500/[0.03] p-3 space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] font-mono text-amber-400/60">
                        {stage === "done" ? `${scenario.fusionTimeMs}ms` : "..."}
                      </span>
                    </div>
                    <div className="space-y-1.5 min-h-[80px]">
                      <AnimatePresence>
              ...
              {stage === "done" && (
                ...
                >
                  ...
                  <div className="rounded-xl border border-white/[0.08] bg-gradient-to-br from-purple-500/[0.04] to-cyan-500/[0.04] p-4 space-y-3">
                    ...
                    <p className="text-xs text-white/50 leading-relaxed">{scenario.insight}</p>
                    ...
                    <div className="flex flex-wrap gap-3 pt-2 border-t border-white/[0.05]">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[10px] font-mono text-emerald-300/60">
                          Total:{" "}
                          {scenario.bm25TimeMs + scenario.vectorTimeMs + scenario.fusionTimeMs}ms
                        </span>
                      </div>
                    </div>
                  </div>
