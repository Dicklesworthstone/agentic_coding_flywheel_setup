...
export function AprLesson() {
  return (
    <div className="space-y-8">
      <GoalBanner>Master AI-powered plan revision with Automated Plan Reviser Pro.</GoalBanner>
      ...
      <Section title="What Is APR?" icon={<RefreshCw className="h-5 w-5" />} delay={0.1}>
        ...
        <Paragraph>
          Early rounds fix architectural issues, middle rounds refine structure, later rounds polish
          abstractions. The process resembles numerical optimization settling into a minimum.
        </Paragraph>
      ...
      <Section title="The Refinement Cycle" icon={<RefreshCw className="h-5 w-5" />} delay={0.15}>
        <Paragraph>
          Watch how APR iteratively improves a specification through multiple passes. Each cycle
          tightens the quality until the plan converges on a polished result.
        </Paragraph>
        <div className="mt-8">
      ...
      <Section title="Essential Commands" icon={<Terminal className="h-5 w-5" />} delay={0.2}>
        <CommandList
          commands={[
            {
              command: "apr setup",
              description: "Configure a workflow (plan file, output dir) once",
            },
            {
              command: "apr run 1",
              description: "Run revision round 1 against the configured workflow",
            },
            {
              command: "apr run 2 --include-impl",
              description: "Next round, bundling the implementation doc",
            },
            { command: "apr diff 2 1", description: "Compare two round outputs" },
            {
              command: "apr integrate 2 -o prompt.md",
              description: "Write the Claude Code integration prompt to a file",
            },
      ...
      <Section title="Typical Workflow" icon={<Play className="h-5 w-5" />} delay={0.3}>
        <CodeBlock
          code={`# 1. Generate initial plan with Claude Code
...
const ITERATION_DATA: IterationData[] = [
    ...
    checks: [
      {
        ...
        status: "fail",
        detail: "No endpoints specified",
      },
      {
        category: "architecture",
        label: "Data models present",
        status: "fail",
        detail: "No schema definitions",
      },
      {
        category: "security",
        label: "Auth strategy defined",
        status: "fail",
        detail: "No token strategy",
      },
    ...
    comments: [
      {
        ...
        text: "Too vague -- needs concrete endpoints, methods, request/response shapes.",
        type: "issue",
      },
      {
        line: 8,
        author: "Security Audit",
        text: '"Handle tokens" is underspecified. Define token type, expiry, rotation strategy.',
        type: "issue",
      },
      {
        line: 12,
        author: "Architecture Review",
        text: '"Store users somewhere" needs a schema with fields and constraints.',
        type: "issue",
      },
    ],
    terminalLines: [
    ...
    checks: [
      {
        ...
        label: "API endpoints defined",
        status: "pass",
        detail: "3 endpoints specified",
      },
      {
        category: "architecture",
        label: "Data models present",
        status: "warn",
        detail: "Partial schema only",
      },
      {
        category: "security",
        label: "Auth strategy defined",
        status: "pass",
        detail: "JWT + refresh token",
      },
      {
        category: "security",
        label: "Input validation",
        status: "fail",
        detail: "No validation rules",
      },
    ...
    comments: [
      {
        line: 6,
        author: "Architecture Review",
        text: "Good. Now add response shapes for each endpoint.",
        type: "suggestion",
      },
      {
        line: 13,
        author: "Security Audit",
        text: "Specify signing algorithm (RS256 vs HS256) and key rotation.",
        type: "suggestion",
      },
      {
        line: 16,
        author: "Architecture Review",
        text: "Add full column definitions, types, and constraints.",
        type: "issue",
      },
    ],
    terminalLines: [
  ...
  },
  {
    ...
    planLines: [
      ...
      {
        text: "- POST /auth/signup {email, password, name} -> {access, refresh}",
        status: "modified",
      },
    ...
    checks: [
      {
        ...
        label: "API endpoints defined",
        status: "pass",
        detail: "Response shapes added",
      },
      {
        category: "architecture",
        label: "Data models present",
        status: "warn",
        detail: "Schema still needs types",
      },
      {
        category: "security",
        label: "Auth strategy defined",
      ...
      },
      {
        ...
        label: "Input validation",
        status: "pass",
        detail: "Rules defined per field",
      },
      {
        category: "performance",
        label: "Rate limiting",
        status: "warn",
        detail: "Mentioned in errors but no config",
      },
      {
        category: "edge-cases",
        label: "Error handling",
        status: "pass",
        detail: "5 error codes documented",
      },
    ],
    comments: [
      {
        line: 12,
        author: "Edge Case Analyzer",
        text: "Good validation. Consider adding password deny-list (common passwords).",
        type: "suggestion",
      },
      {
        line: 20,
        author: "Edge Case Analyzer",
        text: "Rate limit error defined but no rate limit configuration section.",
        type: "issue",
      },
      {
        line: 6,
        author: "Architecture Review",
        text: "Endpoint shapes look solid now.",
        type: "praise",
      },
    ],
    terminalLines: [
  ...
  },
  {
    ...
    planLines: [
      ...
      { text: "## API Endpoints", status: "unchanged" },
      {
        text: "| Method | Path           | Body                    | Response          |",
        status: "modified",
      },
      {
        text: "|--------|----------------|-------------------------|-------------------|",
      ...
      {
        text: "| POST   | /auth/login    | {email, password}       | {access, refresh} |",
        status: "modified",
      },
      {
        text: "| POST   | /auth/signup   | {email, password, name} | {access, refresh} |",
      ...
      {
        text: "| POST   | /auth/refresh  | {refreshToken}          | {access}          |",
        status: "modified",
      },
      {
        text: "| POST   | /auth/logout   | {refreshToken}          | 204 No Content    |",
        status: "added",
      },
      { text: "", status: "unchanged" },
    ...
    checks: [
      {
        ...
        label: "API endpoints defined",
        status: "pass",
        detail: "Table format with logout",
      },
      {
        category: "architecture",
        label: "Data models present",
        status: "pass",
        detail: "Full schema with indices",
      },
      {
        category: "security",
        label: "Auth strategy defined",
      ...
      },
      {
        ...
        label: "Rate limiting",
        status: "pass",
        detail: "Per-endpoint with Redis",
      },
      {
        category: "edge-cases",
        label: "Error handling",
    ...
    comments: [
      {
        ...
        text: "Logout endpoint added -- good for token revocation.",
        type: "praise",
      },
      {
        line: 14,
        author: "Performance Review",
        text: "Redis sliding window is the right choice here.",
        type: "praise",
      },
      {
        line: 20,
        author: "Architecture Review",
        text: "Add created_at/updated_at timestamps and soft delete.",
        type: "suggestion",
      },
    ],
    terminalLines: [
  ...
  },
  {
    ...
    checks: [
      ...
      },
      {
        ...
        label: "Auth strategy defined",
        status: "pass",
        detail: "RS256 with kid rotation",
      },
      {
        category: "security",
        label: "Input validation",
        status: "pass",
        detail: "With password deny-list",
      },
      {
        category: "performance",
        label: "Rate limiting",
        status: "pass",
        detail: "Backoff + account lockout",
      },
      {
        category: "edge-cases",
        label: "Error handling",
        status: "pass",
        detail: "Generic messages, no leaks",
      },
    ],
    comments: [
      {
        line: 16,
        author: "Security Audit",
        text: "Refresh token family revocation is excellent for detecting token theft.",
        type: "praise",
      },
      {
        line: 14,
        author: "Security Audit",
        text: "bcrypt(12) + common password reject -- solid password policy.",
        type: "praise",
      },
      {
        line: 22,
        author: "Security Audit",
        text: "Account lockout prevents brute-force while backoff slows automation.",
        type: "praise",
      },
    ],
    terminalLines: [
    ...
    checks: [
      {
        ...
        label: "API endpoints defined",
        status: "pass",
        detail: "Complete with all verbs",
      },
      {
        category: "architecture",
        label: "Data models present",
        status: "pass",
        detail: "Full schema, indices, FK",
      },
      {
        category: "security",
        label: "Auth strategy defined",
        status: "pass",
        detail: "RS256, rotation, lockout",
      },
      {
        category: "security",
        label: "Input validation",
        status: "pass",
        detail: "Comprehensive rules",
      },
      {
        category: "performance",
        label: "Rate limiting",
    ...
    comments: [
      {
        line: 16,
        author: "Final Review",
        text: "Observability section ensures production debuggability.",
        type: "praise",
      },
      {
        line: 21,
        author: "Final Review",
        text: "Deployment spec with health checks -- ready for k8s.",
        type: "praise",
      },
      {
        line: 0,
        author: "APR Engine",
        text: "Converged. Quality delta < 3% -- no further passes needed.",
        type: "praise",
      },
    ],
    terminalLines: [
          ...
          return (
            ...
            >
              <div className={`flex items-start px-1 ${getLineBg(line.status)}`}>
                ...
                {/* Diff prefix */}
                <span
                  className={`w-4 text-center text-[10px] font-mono ${prefix.color} select-none py-0.5 flex-shrink-0`}
                >
                  {prefix.char}
                </span>
                {/* Content */}
            ...
            <div className="rounded-xl border border-white/[0.08] bg-black/20 overflow-hidden">
              ...
              <div className="flex border-b border-white/[0.06]">
                {(
                  [
                    {
                      key: "checks" as ActiveTab,
                      label: "Checks",
                      icon: <Eye className="h-3 w-3" />,
                      badge: `${passCount}/${totalChecks}`,
                    },
                    {
                      key: "terminal" as ActiveTab,
                      label: "Terminal",
                      icon: <Terminal className="h-3 w-3" />,
                      badge: null,
                    },
                    {
                      key: "comments" as ActiveTab,
                      label: "Reviews",
                ...
                <AnimatePresence mode="wait">
                  ...
                  {activeTab === "terminal" && (
                    <motion.div
                      ...
                      transition={{ type: "spring", stiffness: 200, damping: 25 }}
                    >
                      <MiniTerminal lines={iteration.terminalLines} animateIn={true} />
                    </motion.div>
                  )}
                  {activeTab === "comments" && (
                    <motion.div
