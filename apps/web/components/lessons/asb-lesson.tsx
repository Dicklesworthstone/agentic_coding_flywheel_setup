  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="What Is ASB?" icon={<Save className="h-5 w-5" />} delay={0.1}>
        <Paragraph>
          <Highlight>ASB (Agent Settings Backup)</Highlight> is a smart backup tool for AI coding
          agent configuration folders. It backs up settings for Claude Code, Cursor, Codex CLI,
          Antigravity CLI, and other agents to a single versioned archive.
        </Paragraph>
        <Paragraph>
          When you provision a new machine or recover from a misconfiguration, ASB restores all your
          agent settings, skills, hooks, and preferences in one command.
        </Paragraph>
      ...
      <Section title="Essential Commands" icon={<Terminal className="h-5 w-5" />} delay={0.2}>
        ...
        <TipBox>
          Run ASB backup before provisioning new machines or making major configuration changes to
          your agent setups.
        </TipBox>
      </Section>
      ...
      <Section title="Common Scenarios" icon={<Play className="h-5 w-5" />} delay={0.3}>
        <CodeBlock
          code={`# Back up all agent configs
asb backup
...
const AGENTS: AgentConfig[] = [
  {
    ...
    files: ["CLAUDE.md", "hooks.json", "settings.json"],
    sizeMB: 2.4,
  },
  {
    name: "Cursor",
    ...
    color: "text-blue-400",
    bgColor: "bg-blue-500",
    files: ["settings.json", "rules/", "mcp.json"],
    sizeMB: 1.8,
  },
  {
    name: "Codex CLI",
    ...
    files: ["config.yaml", "instructions.md"],
    sizeMB: 0.9,
  },
  {
    ...
    files: ["settings.json", "hooks.json", "AGENTS.md"],
    sizeMB: 1.1,
  },
  {
    name: "Aider",
...
const SCENARIOS: Scenario[] = [
  {
    ...
    description:
      "Automated daily backup captures all agent configs into a timestamped snapshot archive.",
    ...
    highlights:
      "All configs captured on schedule with automatic compression and checksum verification.",
  },
  {
    id: "pre-deploy",
    ...
    highlights:
      "Tagged snapshots let you label important milestones and roll back to exact known states.",
  },
  {
    id: "disaster",
  ...
  return (
    <svg ref={rootRef} viewBox="0 0 480 140" className="w-full h-auto" aria-hidden="true">
      ...
      <g opacity={arrowOpacity}>
        ...
        />
        <text x={238} y={62} textAnchor="middle" className="fill-white/30 text-[8px] font-mono">
      ...
      {isVerify && phase === "done" && (
        ...
        >
          ...
          <text
            ...
            y={120}
            textAnchor="middle"
            className="fill-amber-400/80 text-[8px] font-mono"
          >
            1 drift detected
          </text>
        </motion.g>
            ...
            >
              ...
              <div className="flex-1 h-5 relative">
                ...
                <div className="absolute inset-0 flex items-center px-2">
                  <span className="text-[10px] font-mono text-white/50">{entry.sizeMB} MB</span>
                </div>
              </div>
              {entry.tagged && (
        ...
        return (
          ...
          >
            ...
            <motion.span
              ...
              animate={
                inView && revealed && !row.match ? { opacity: [0.5, 1, 0.5] } : { opacity: 1 }
              }
              transition={
                inView && revealed && !row.match ? { duration: 1.5, repeat: Infinity } : {}
              }
            >
...
function RetentionChart({ visible }: { visible: boolean }) {
  ...
  return (
    <div className="space-y-2">
      <div className="text-[10px] text-white/40 font-mono px-1">Retention Policy</div>
      <div className="space-y-2">
        {bars.map((bar, i) => (
          <motion.div
            key={bar.label}
...
function MiniTerminal({ lines, visibleCount }: { lines: string[]; visibleCount: number }) {
  ...
  return (
    <div
      ref={rootRef}
      className="rounded-xl border border-white/[0.08] bg-black/40 overflow-hidden"
    >
      {/* Title bar */}
      ...
      <div
        ref={scrollRef}
        className="p-3 font-mono text-[10px] leading-relaxed space-y-0.5 max-h-48 overflow-y-auto"
      >
          ...
          {displayed.map((line, i) => (
            <motion.div
              ...
              className={
                line.startsWith("$")
                  ? "text-emerald-400/80"
                  : line.includes("DRIFT") || line.includes("drift")
                    ? "text-amber-400/80"
                    : line.includes("restored") ||
                        ...
                        line.includes("OK")
                      ? "text-emerald-400/60"
                      : line.includes("Agent") || line.includes("---")
                        ? "text-white/50"
                        : line === ""
                          ? "h-2"
                          : "text-white/35"
...
function InteractiveBackupRestoreImpl() {
  ...
  return (
    ...
    >
      {/* Background glows */}
      <div
        className={`absolute top-0 left-1/4 w-64 h-64 rounded-full blur-3xl pointer-events-none transition-colors duration-700 ${
          scenarioGlowColor === "emerald"
            ? "bg-emerald-500/[0.06]"
            : scenarioGlowColor === "blue"
              ? "bg-blue-500/[0.06]"
              : scenarioGlowColor === "red"
                ? "bg-red-500/[0.06]"
                : scenarioGlowColor === "violet"
                  ? "bg-violet-500/[0.06]"
                  : scenarioGlowColor === "cyan"
                    ? "bg-cyan-500/[0.06]"
                    : scenarioGlowColor === "amber"
                      ? "bg-amber-500/[0.06]"
                      : "bg-emerald-500/[0.06]"
        }`}
      />
      ...
      <div className="relative p-6 space-y-5">
        ...
        <div className="rounded-xl border border-white/[0.06] bg-black/20 p-3">
          ...
          <VpsStateDiagram phase={phase} scenarioId={scenario.id} progress={progress} />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {/* Terminal */}
          <div>
            <MiniTerminal lines={scenario.terminalLines} visibleCount={terminalVisibleCount} />
          </div>
              ...
              <div className="rounded-xl border border-white/[0.06] bg-black/20 p-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {AGENTS.map((agent, i) => {
                    ...
                    const isIncluded =
                      !isSelective || agent.shortName === "claude" || agent.shortName === "cursor";
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div className="min-w-0 flex-1">
                          <div className={`text-[10px] font-semibold ${agent.color}`}>
                            {agent.shortName}
                          </div>
        ...
        <div className="text-center text-xs text-white/25">
          {phase === "idle" &&
            `Click "Run Scenario" to watch ${scenario.label.toLowerCase()} in action`}
