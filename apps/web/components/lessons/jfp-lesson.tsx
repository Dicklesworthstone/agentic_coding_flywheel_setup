  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="What Is JFP?" icon={<GraduationCap className="h-5 w-5" />} delay={0.1}>
        <Paragraph>
          <Highlight>JFP (JeffreysPrompts.com CLI)</Highlight> gives you access to a curated library
          of battle-tested prompts for Claude, GPT, and other AI coding agents. Browse, copy, and
      ...
      <Section
        title="Prompt Crafting Laboratory"
      ...
      <Section title="Essential Commands" icon={<Terminal className="h-5 w-5" />} delay={0.2}>
        <CommandList
          commands={[
            ...
            {
              command: "jsm install <id>",
              description:
                "Install as a Claude Code skill (skill management moved from jfp to jsm)",
            },
        ...
        <TipBox variant="info">
          Visit{" "}
          <a href="https://jeffreysprompts.com" className="text-blue-400 underline">
            jeffreysprompts.com
          </a>{" "}
          for a beautiful UI to browse prompts.
        </TipBox>
      </Section>
...
const TEMPLATE_SECTIONS: TemplateSectionData[] = [
  {
    ...
    label: "Role",
    icon: "user",
    text: "You are a senior {{ROLE}} engineer with expertise in {{TECH_STACK}}.",
    color: "text-violet-400",
  },
  {
    id: "task",
    label: "Task",
    icon: "target",
    text: "{{ACTION}} the {{COMPONENT}} in {{FILE_PATH}}.",
    color: "text-emerald-400",
  },
  {
    id: "context",
  ...
  },
  {
    ...
    label: "Examples",
    icon: "code",
    text: "Example input: {{EXAMPLE_INPUT}}\nExample output: {{EXAMPLE_OUTPUT}}",
    color: "text-cyan-400",
  },
];

const VAGUE_VS_PRECISE: {
  vague: string;
  precise: string;
  category: PromptCategory;
  vagueResult: number;
...
}[] = [
  {
    ...
    vague: "Fix the crash",
    precise:
      "Fix the null pointer dereference in UserService.getProfile() when user.address is undefined (line 142)",
    vagueResult: 25,
    preciseResult: 92,
  },
  {
    ...
    vague: "Add dark mode",
    precise:
      "Add dark mode toggle using next-themes with system preference detection, persisted to localStorage, applied via Tailwind dark: prefix",
    vagueResult: 35,
    preciseResult: 90,
  },
  {
    category: "Refactor",
    vague: "Make this cleaner",
    precise:
      "Extract the 3 repeated validation blocks (lines 45, 89, 134) into a shared validateInput() utility in lib/validation.ts",
    vagueResult: 20,
    preciseResult: 88,
  },
  {
    ...
    precise:
      "Write Vitest unit tests for calculateDiscount() covering: percentage caps, negative values, expired coupons, and stacked discounts",
    vagueResult: 30,
    preciseResult: 95,
  },
  {
    category: "Code Review",
    vague: "Check for issues",
    precise:
      "Review for N+1 query patterns in the dashboard data loader, specifically the user.posts.comments chain in getDashboardData()",
    vagueResult: 22,
    preciseResult: 91,
  },
  {
    category: "Architecture",
    vague: "Design the backend",
    precise:
      "Design a CQRS event-sourced order service with separate read/write models, Kafka event bus, and PostgreSQL + Redis projections",
    vagueResult: 18,
    preciseResult: 93,
  },
];
...
const TERMINAL_LINES = [
  ...
  { type: "output" as const, text: "" },
  {
    ...
    text: "  1. perf-review-pro    Performance-focused code review     4.9",
  },
  {
    type: "result" as const,
    text: "  2. bundle-analyzer    Bundle size optimization review      4.7",
  },
  {
    type: "result" as const,
    text: "  3. render-audit       React re-render analysis             4.6",
  },
  { type: "output" as const, text: "" },
  ...
  {
    type: "agent" as const,
    text: "  2. CartSummary missing React.memo - 23 unnecessary re-renders",
  },
...
function PromptEvolutionTab() {
  ...
  function highlightText(text: string, highlights: string[]) {
    ...
    for (const hl of highlights) {
      ...
      if (idx > 0) {
        parts.push(
          <span key={`t-${keyIdx}`} className="text-white/60">
    ...
    if (remaining) {
      parts.push(
        <span key={`r-${keyIdx}`} className="text-white/60">
          {remaining}
        </span>,
      );
    }
  ...
  const overallScore = Math.round(
    (version.scores.clarity +
      version.scores.specificity +
      version.scores.context +
      version.scores.constraints +
      version.scores.examples) /
      5,
  );
          ...
          <div className="flex items-center gap-2">
            {evo.versions.map((v, vi) => (
              <div key={v.version} className="flex items-center gap-2">
                ...
                {vi < 2 && <ArrowRight className="h-3 w-3 text-white/20 shrink-0" />}
              </div>
            ))}
          </div>
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="flex flex-wrap gap-1 pt-1">
                  {["Ambiguous scope", "No context", "Missing constraints"].map((tag) => (
                    <span
                      key={tag}
                      className="px-1.5 py-0.5 rounded text-[10px] bg-red-500/10 text-red-400/70 border border-red-500/20"
                    >
                      {tag}
                    </span>
                  ))}
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <div className="flex flex-wrap gap-1 pt-1">
                  {["Specific location", "Clear context", "Defined output"].map((tag) => (
                    <span
                      key={tag}
                      className="px-1.5 py-0.5 rounded text-[10px] bg-emerald-500/10 text-emerald-400/70 border border-emerald-500/20"
                    >
                      {tag}
                    </span>
                  ))}
  ...
  return (
    <div className="space-y-5">
      <p className="text-xs text-white/40">
        Drag to reorder sections. Toggle to include or exclude. Watch the assembled prompt and
        quality score update in real time.
      </p>
          ...
          {sectionOrder.map((sectionId) => {
            ...
            return (
              ...
              >
                ...
                <span className="flex-1 text-[10px] text-white/30 truncate font-mono">
                  {section.text.split("\n")[0]}
                </span>
              </motion.div>
            );
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-xl bg-black/40 border border-white/[0.06] p-4 max-h-[320px] overflow-y-auto">
            <AnimatePresence mode="wait">
              ...
              >
                {builtPrompt ? (
                  <pre className="font-mono text-xs leading-relaxed text-white/60 whitespace-pre-wrap">
                    {builtPrompt}
                  </pre>
                ) : (
                  <p className="text-xs text-white/30 italic">
                    Enable sections to build your prompt template...
                  </p>
                )}
              </motion.div>
            </AnimatePresence>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] text-white/30">
              {enabledSections.size} of {TEMPLATE_SECTIONS.length} sections active
            </span>
            <div className="flex-1" />
  ...
  return (
    <div ref={rootRef} className="space-y-4">
      ...
      <div className="rounded-xl border border-white/[0.08] bg-black/60 overflow-hidden">
        ...
        <div
          ref={terminalRef}
          className="p-4 font-mono text-xs leading-relaxed max-h-[360px] overflow-y-auto space-y-0.5"
        >
      ...
      <div className="flex items-center gap-4 text-[10px] text-white/30 px-1">
        <span>
          Lines: {visibleLines}/{TERMINAL_LINES.length}
        </span>
        <div className="flex-1 h-px bg-white/[0.06]" />
        <span
          className={`flex items-center gap-1 ${isRunning ? "text-emerald-400/70" : "text-white/30"}`}
        >
          <div
            className={`h-1.5 w-1.5 rounded-full ${isRunning ? "bg-emerald-400" : "bg-white/20"}`}
          />
