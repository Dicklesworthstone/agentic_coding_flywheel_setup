  ...
  return (
    <div className="space-y-8">
      ...
      <Section title="The Debugging Toolkit" icon={<Bug className="h-5 w-5" />} delay={0.1}>
        <Paragraph>
          When agents stop working, you need to answer:{" "}
          <Highlight>
      ...
      <Section
        title="Rate Limit Diagnosis"
        icon={<AlertTriangle className="h-5 w-5" />}
        delay={0.15}
      >
        <Paragraph>
          Rate limits are the most common agent failure. Here&apos;s how to diagnose and work around
          them.
        </Paragraph>
...
cass search "rate limit workaround rotation" --limit 3`}
        ...
        <TipBox variant="tip">
          If you see 429s from RANO but CAUT shows low token usage, you&apos;re likely hitting
          requests-per-minute limits, not token limits. Space out your requests or use CAAM to
          rotate accounts.
        </TipBox>
      </Section>
      ...
      <Section title="Network Debugging" icon={<Wifi className="h-5 w-5" />} delay={0.2}>
        <Paragraph>
          RANO intercepts all agent API traffic, letting you see exactly what&apos;s happening at
          the network level.
        </Paragraph>
        ...
        <CommandList
          commands={[
            ...
            {
              command: "rano log --slow --threshold 10s",
              description: "Find requests taking over 10 seconds",
            },
...
# SSL errors → Certificate issue (check system time/certs)`}
      ...
      <Section title="Cost Investigation" icon={<DollarSign className="h-5 w-5" />} delay={0.25}>
        <Paragraph>
          When costs spike unexpectedly, use CAUT to find the expensive sessions and RANO to see
          what they&apos;re doing.
        </Paragraph>
...
# 4. Using Opus when Sonnet would suffice`}
        ...
        <TipBox variant="warning">
          If an agent&apos;s input tokens are 10x its output tokens, it&apos;s probably stuffing too
          much context. Use TRU to compress source files before feeding them to agents.
        </TipBox>
      </Section>
...
wa robot send <pane_id> "Continue with the task"`}
      ...
      <Section title="Learning from History" icon={<Search className="h-5 w-5" />} delay={0.35}>
        <Paragraph>
          CASS stores all past agent sessions. When you encounter a new problem, search for how you
          (or another agent) solved it before.
        </Paragraph>
        ...
        <div className="mt-6 grid gap-3 sm:grid-cols-2">
          <div className="p-3 rounded-lg bg-cyan-500/10 border border-cyan-500/30">
            ...
            <p className="text-white/80 text-sm mt-1">
              RANO status + CAUT summary = instant health check
            </p>
          </div>
          ...
          <div className="p-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30">
            ...
            <p className="text-white/80 text-sm mt-1">
              CAUT breakdown + session drill-down = spend accountability
            </p>
          </div>
          <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
            <span className="text-amber-400 font-semibold">Pattern Recognition</span>
            <p className="text-white/80 text-sm mt-1">
              CASS history + CM playbook = never solve twice
            </p>
          </div>
        </div>
      </Section>
  ...
  return [
    {
      ...
      stackFrames: [
        {
          ...
          file: "agent/main.ts",
          isError: true,
          description: "runLoop() called 47 times without progress",
        },
        {
          layer: "Tool Call",
          file: "tools/edit.ts",
          isError: true,
          description: "Same edit applied and reverted repeatedly",
        },
        {
          layer: "File System",
          file: "fs/write.ts",
          isError: false,
          description: "Write succeeds but lint fails",
        },
        {
          layer: "Lint Check",
          file: "lint/eslint.ts",
          isError: true,
          description: "Error persists after each edit attempt",
        },
      ],
      agentView: [
      ...
      fixes: [
        ...
        {
          rank: 2,
          label: "Add eslint-disable for conflicting rule",
          command: 'Agent: "Add eslint-disable comment"',
          confidence: 82,
        },
        {
          rank: 3,
      ...
      sessionLog: [
        ...
        { time: "14:03:45", level: "warn", message: "Loop detected: 12 identical edits" },
        {
          time: "14:05:30",
          level: "error",
          message: "Context 97% full, 47 turns with no progress",
        },
      ],
    },
    {
      ...
      stackFrames: [
        {
          ...
          isError: true,
          description: "Stale file content from 28 turns ago",
        },
        {
          layer: "File Read",
          file: "tools/read.ts",
          isError: false,
          description: "Last read at turn 3, file changed at turn 15",
        },
        {
          layer: "Git State",
          file: "git/status.ts",
          isError: true,
          description: "Branch diverged, agent unaware",
        },
        {
          layer: "Edit Engine",
          file: "tools/edit.ts",
          isError: false,
          description: "Edits based on outdated file content",
        },
      ],
      agentView: [
      ...
      sessionLog: [
        { time: "13:45:00", level: "info", message: "Agent read src/api.ts (snapshot stored)" },
        {
          time: "13:52:00",
          level: "info",
          message: "Pane-3 agent committed changes to src/api.ts",
        },
    ...
    },
    {
      ...
      stackFrames: [
        {
          ...
          file: "tools/resolve.ts",
          isError: true,
          description: "Ambiguous path resolved to wrong file",
        },
        {
          layer: "Search",
          file: "tools/grep.ts",
          isError: false,
          description: "Found 3 files matching pattern",
        },
        {
          layer: "Edit Engine",
          file: "tools/edit.ts",
          isError: false,
          description: "Edit succeeded on wrong target",
        },
        {
          layer: "Test Runner",
          file: "test/runner.ts",
          isError: true,
          description: "Unrelated test broke from wrong edit",
        },
      ],
      agentView: [
      ...
      fixes: [
        ...
        {
          rank: 3,
          label: "Add path context to prompt",
          command: 'cm playbook add "Always use full paths" --category workflow',
          confidence: 72,
        },
      ],
      sessionLog: [
    ...
    },
    {
      ...
      stackFrames: [
        {
          ...
          file: "test/suite.ts",
          isError: true,
          description: "23 tests failing from 1 root cause",
        },
        {
          layer: "Shared Fixture",
          file: "test/fixtures.ts",
          isError: true,
          description: "Setup function returns wrong shape",
        },
        {
          layer: "Type Layer",
          file: "types/api.ts",
          isError: false,
          description: "Interface changed, fixtures not updated",
        },
        {
          layer: "Agent Focus",
          file: "agent/context.ts",
          isError: true,
          description: "Agent fixing tests one-by-one instead of root",
        },
      ],
      agentView: [
      ...
      sessionLog: [
        ...
        { time: "11:05:00", level: "info", message: "Agent fixing test #8 individually" },
        {
          time: "11:12:00",
          level: "warn",
          message: "Pattern detected: same null check added 8 times",
        },
    ...
    },
    {
      ...
      stackFrames: [
        {
          layer: "Rate Limiter",
          file: "api/rateLimit.ts",
          isError: true,
          description: "Provider returning 429 on every request",
        },
        {
          layer: "Retry Logic",
          file: "api/retry.ts",
          isError: true,
          description: "Exponential backoff not increasing fast enough",
        },
        {
          layer: "Agent Scheduler",
          file: "agent/schedule.ts",
          isError: false,
          description: "3 agents hitting same provider simultaneously",
        },
        {
          layer: "Account Pool",
          file: "auth/caam.ts",
          isError: true,
          description: "All accounts in pool exhausted",
        },
      ],
      agentView: [
        "Sending API request...",
      ...
      fixes: [
        {
          rank: 1,
          label: "Rotate API accounts",
          command: "caam next claude --usage-aware",
          confidence: 94,
        },
        {
          rank: 2,
      ...
      sessionLog: [
        ...
        { time: "09:35:00", level: "error", message: "All CAAM accounts exhausted" },
        {
          time: "09:36:00",
          level: "error",
          message: "Agents fully blocked, 0 successful requests",
        },
      ],
    },
    {
      ...
      stackFrames: [
        {
          ...
          file: "git/merge.ts",
          isError: true,
          description: "Merge conflict in 3 files",
        },
        {
          layer: "Agent A Edit",
          file: "src/auth.ts",
          isError: true,
          description: "Refactored function signature",
        },
        {
          layer: "Agent B Edit",
          file: "src/auth.ts",
          isError: true,
          description: "Added parameter to same function",
        },
        {
          layer: "Coordination",
          file: "agents/mail.ts",
          isError: true,
          description: "No lock acquired before edits",
        },
      ],
      agentView: [
      ...
      fixes: [
        {
          ...
          command: "git mergetool src/auth.ts",
          confidence: 97,
        },
        {
          rank: 2,
          label: "Add file locking via Agent Mail",
          command:
            'am mail send --project ~/projects/myproject --from cc-1 --to cc-2,cc-3 --subject "Lock src/auth.ts" --body "Reserve src/auth.ts before editing"',
          confidence: 85,
        },
        {
          rank: 3,
          label: "Assign files to specific agents",
          command: 'cm playbook add "Only pane-1 edits src/auth.ts" --category coordination',
          confidence: 79,
        },
      ],
      sessionLog: [
        ...
        { time: "16:03:00", level: "error", message: "Merge conflict in src/auth.ts" },
        {
          time: "16:03:15",
          level: "error",
          message: "CI pipeline failed: conflict markers in source",
        },
      ],
    },
  ];
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between">
        ...
        <span className="text-[10px] font-mono text-white/60">{animated ? formatted : "--"}</span>
      </div>
  ...
  return (
    <div className="space-y-1.5">
      {fixes.map((fix, i) => (
        ...
        >
          <div
            className={`mt-0.5 flex h-5 w-5 items-center justify-center rounded-full text-[10px] font-bold shrink-0 ${
              fix.rank === 1
      ...
      <div className="relative p-4 sm:p-6 space-y-5">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-white/50">
            Step through 6 real debugging scenarios. Click &ldquo;Diagnose&rdquo; to analyze each
            one.
          </p>
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="flex items-center gap-1.5 text-[10px] text-white/40">
              <span>
                Scenario {activeIdx + 1}/{scenarios.length}
              </span>
            </div>
          </motion.div>
        </AnimatePresence>
                  ...
                  >
                    ...
                    <p className="text-xs text-white/70 leading-relaxed">
                      ...
                      <span className="text-emerald-400 font-mono">
                        {scenario.fixes[0]?.confidence ?? 0}% confidence
                      </span>
                      )
                    </p>
                  </motion.div>
