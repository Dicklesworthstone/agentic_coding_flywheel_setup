...
function fixture() {
  const entries = new Map(); const cache = new Map(); const events = [];
  const listeners = new Map(); const writes = []; const analytics = []; const copies = [];
  const hooks = []; let cursor = 0; let effects = []; let latestMutation; const pending = [];
  let queryStatus = 'success'; let storageAvailable = true; let completeCalls = 0;
  const same = (a, b) => a && b && a.length === b.length && a.every((v, i) => Object.is(v, b[i]));
  const jsx = (type, props) => ({ type, props: props ?? {} });
  const keyFor = (queryKey) => JSON.stringify(queryKey);
  const queryClient = { setQueryData(key, value) { cache.set(keyFor(key), value); writes.push({ key, value }); } };
  const dependencies = {
    react: {
      useState(initial) { const at = cursor++; hooks[at] ??= { value: initial }; return [hooks[at].value, (value) => { hooks[at].value = value; }]; },
      useRef(initial) { const at = cursor++; hooks[at] ??= { current: initial }; return hooks[at]; },
      useCallback(fn) { cursor++; return fn; },
      useEffect(fn, deps) { const at = cursor++; if (!hooks[at] || !same(deps, hooks[at].deps)) {
        const previous = hooks[at]?.cleanup; hooks[at] = { deps };
        effects.push(() => { previous?.(); hooks[at].cleanup = fn(); });
      } },
    },
    ...
    '@tanstack/react-query': {
      ...
      useQuery(options) {
        ...
        if (options.enabled !== false && queryStatus === 'success' && !cache.has(key)) cache.set(key, options.queryFn());
      ...
      useMutation(options) { latestMutation = options; return { mutate(value) {
        const result = options.mutationFn(value);
        pending.push(async () => latestMutation.onSuccess(await result));
      } }; },
    },
    'lucide-react': Object.fromEntries(['Check', 'Copy', 'Terminal', 'CheckCircle2', 'Server', 'Monitor'].map((name) => [name, name])),
    '@/components/motion': { motion: { div: 'motion-div' }, AnimatePresence: 'presence', springs: { snappy: {} } },
    ...
    '@/lib/utils': { cn: (...items) => items.filter(Boolean).join(' '),
      ...
      safeSetItem: (key, value) => { if (!storageAvailable) return false; entries.set(key, value); return true; } },
    '@/lib/userPreferences': { useUserOS: () => ['linux'], useDetectedOS: () => 'linux' },
    '@/lib/hooks/useReducedMotion': { useReducedMotion: () => false },
    '@/lib/hooks/useCopyFeedback': { useCopyFeedback: () => ({ state: 'idle', copy: async (value) => { copies.push(value); return true; } }) },
    '@/lib/analytics': { commandCopyAnalyticsProperties: (value) => ({ length: value.length }), trackInteraction: (...args) => analytics.push(args) },
  };
  ...
  const dispatch = (event) => { events.push(event); for (const listener of listeners.get(event.type) ?? []) listener(event); };
  ...
  const props = { command: 'acfs doctor', showCheckbox: true,
    persistKey: `flywheel-doctor-v2-${'a'.repeat(64)}`, checkboxId: 'flywheel-doctor', onComplete: () => { completeCalls++; } };
  function render() {
    cursor = 0; effects = []; const nodes = [];
    function visit(node) {
    ...
    visit(module.exports.CommandCard(props)); effects.forEach((effect) => effect());
    return { nodes, checkbox: () => nodes.find((node) => node.type === 'checkbox'),
      label: () => nodes.find((node) => node.type === 'label'),
      copy: () => nodes.find((node) => node.type === 'button') };
  }
  return { props, entries, cache, writes, events, copies, analytics, listeners, render, dispatch,
    completeCalls: () => completeCalls, flush: async () => { while (pending.length) await pending.shift()(); },
    status: (value) => { queryStatus = value; }, storage: (value) => { storageAvailable = value; },
...
test('stable accessible checkbox ID stays separate from its opaque context key', () => {
  const f = fixture(); const v = f.render();
...
test('local acknowledgement completes synchronously and cannot settle into a later context', async () => {
  const f = fixture(); const oldKey = f.props.persistKey;
...
test('unchecking cannot be overwritten by an older completion microtask', async () => {
  const f = fixture(); f.render().checkbox().props.onCheckedChange(true);
...
for (const status of ['pending', 'error']) {
  test(`cannot acknowledge a context before its storage query is ready: ${status}`, async () => {
    const f = fixture(); f.status(status); const view = f.render();
    assert.equal(view.checkbox().props.disabled, true);
    view.checkbox().props.onCheckedChange(true); await f.flush();
    assert.equal(f.entries.size, 0); assert.equal(f.completeCalls(), 0);
  });
}

test('indeterminate is not a success acknowledgement', () => {
  const f = fixture(); f.render().checkbox().props.onCheckedChange('indeterminate');
  assert.equal(f.entries.get(`acfs-command-${f.props.persistKey}`), 'false');
  assert.equal(f.completeCalls(), 0);
});

test('storage clear events revoke current completion', async () => {
  const f = fixture(); f.render().checkbox().props.onCheckedChange(true); await f.flush();
  assert.equal(f.render().checkbox().props.checked, true);
  f.entries.clear(); f.dispatch({ type: 'storage', key: null, newValue: null });
  assert.equal(f.render().checkbox().props.checked, false);
});
...
test('ignores completion events for another key or with nonboolean values', () => {
  const f = fixture(); f.render();
  f.dispatch({ type: 'acfs:command-completion-changed', detail: { key: `acfs-command-${f.props.persistKey}`, completed: 'true' } });
  assert.equal(f.render().checkbox().props.checked, false);
  f.dispatch({ type: 'acfs:command-completion-changed', detail: { key: 'other', completed: true } });
  assert.equal(f.render().checkbox().props.checked, false);
});

test('same-tab checked state works when browser storage refuses writes', async () => {
  const f = fixture(); f.storage(false); f.render().checkbox().props.onCheckedChange(true); await f.flush();
  assert.equal(f.render().checkbox().props.checked, true); assert.equal(f.entries.size, 0);
});

test('leaving a card removes its synchronization listeners', () => {
  const f = fixture(); f.render();
  assert.equal(f.listeners.get('storage').size, 1);
  f.unmount(); assert.equal(f.listeners.get('storage').size, 0);
...
test('unscoped cards retain their existing IDs and command copy behavior', async () => {
  const f = fixture(); delete f.props.checkboxId; f.props.persistKey = 'tutorial-command';
  const view = f.render(); assert.equal(view.checkbox().props.id, 'tutorial-command');
  await view.copy().props.onClick(); assert.deepEqual(f.copies, ['acfs doctor']);
});
