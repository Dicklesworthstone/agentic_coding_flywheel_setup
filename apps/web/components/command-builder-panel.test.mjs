...
function loadSource(url, dependencies = {}) {
  ...
  const run = runInContext(`(function(require, module, exports) {\n${compiled.outputText}\n})`, context,
...
function fixture() {
  let cursor = 0; let id = 0; const states = [];
  const preferences = { ip: '203.0.113.42', os: 'linux', mode: 'safe', username: 'ubuntu', ref: null, profile: 'full' };
  const buildCalls = []; const shares = []; const copies = [];
  const jsx = (type, props) => ({ type, props: props ?? {} });
  const preference = (key) => () => [preferences[key], (value) => { preferences[key] = value; }];
  const component = loadSource(new URL('./command-builder-panel.tsx', import.meta.url), {
    ...
    '@/lib/userPreferences': {
      useVPSIP: preference('ip'), useUserOS: preference('os'), useInstallMode: preference('mode'),
      useSSHUsername: preference('username'), useACFSRef: preference('ref'), useModuleProfile: preference('profile'),
    ...
    '@/lib/commandBuilder': {
      buildCommands(input) {
        ...
        return [{ id: 'installer', label: 'Run installer', description: 'Fixture command row', runLocation: 'vps',
          command: `bash install.sh --mode ${input.mode} ${selection.buildInstallSelectorArgs(input.moduleSelection).join(' ')}` }];
      },
      buildShareURL(input) { shares.push(plain(input)); return 'https://example.invalid/profile'; },
    },
  });
  function render() {
    cursor = 0; id = 0;
    ...
    const find = (predicate) => { const node = nodes.find(predicate); assert.ok(node, 'expected component element'); return node; };
    return {
      nodes,
      command: () => nodes.filter((node) => node.type === 'code').map((node) => text(node)).join('\n'),
      button: (label) => find((node) => node.type === 'button'
      ...
      alert: () => nodes.filter((node) => node.props.role === 'alert').map(text).join('\n'),
    };
  }
...
test('panel pickers derive all categories and tags from the production catalogue', () => {
  ...
  for (const kind of ['category', 'tag']) {
    ...
    assert.deepEqual(new Set(children.filter((node) => node.props.value).map((node) => node.props.value)), expected);
...
test('adding and removing groups updates the copyable exact installer flags', async () => {
  const item = fixture();
  item.render().picker('tag').props.onChange({ target: { value: 'maintenance' } });
...
test('a dependency conflict suppresses command building without throwing or granting noDeps', () => {
  const item = fixture();
  item.render().picker('category').props.onChange({ target: { value: 'lang' } });
  const previous = item.buildCalls.length; const view = item.render();
  assert.match(view.alert(), /Install plan blocked/);
  assert.match(view.alert(), /depends on skipped/);
  assert.equal(view.command(), '');
  assert.equal(item.buildCalls.length, previous, 'invalid selections must not reach the command builder');
  assert.equal(view.button('Share link').props.disabled, true);
  view.button('Clear all exclusions').props.onClick();
...
test('switching to a conflicting explicit profile blocks commands until exclusion removal', () => {
  const item = fixture();
  item.render().picker('category').props.onChange({ target: { value: 'cloud' } });
  assert.ok(item.render().command());
  item.preferences.profile = 'cloud-only';
  const view = item.render();
  assert.equal(view.command(), ''); assert.match(view.alert(), /was requested/);
...
test('the panel can preview exclusions without a stored host and rejects unknown option values', () => {
  const item = fixture(); item.preferences.ip = '';
  item.render().picker('tag').props.onChange({ target: { value: 'maintenance' } });
  let view = item.render();
  assert.equal(view.command(), ''); assert.equal(item.buildCalls.length, 0);
  view.picker('tag').props.onChange({ target: { value: 'unknown-tag' } });
  item.preferences.ip = '203.0.113.42'; view = item.render();
  assert.match(view.command(), /acfs.nightly/); assert.doesNotMatch(view.command(), /unknown-tag/);
