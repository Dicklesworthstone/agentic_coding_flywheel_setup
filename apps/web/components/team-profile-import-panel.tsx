...
import {
  approveTeamProfileReview, reviewTeamProfileFile, teamProfileReviewMatches,
  TeamProfileImportError, type ApprovedTeamProfileCommand, type TeamProfileFileReview,
...
function Changes({ title, changes }: { title: string; changes: TeamProfileImportChange[] }) {
  return <div className="space-y-2">
    ...
    {changes.length === 0 ? <p className="text-sm text-muted-foreground">No changes.</p>
      : <dl className="space-y-2 text-sm">{changes.map((change) => <div key={change.field}>
...
export function TeamProfileImportPanel() {
  ...
  const id = useId();
  const ready = modeLoaded && profileLoaded && refLoaded && usernameLoaded && hostLoaded && providerLoaded;
  ...
  const context = useMemo<TeamProfileReviewContext | null>(() => {
    ...
    return { targetHost: host, current: { providerSelection: provider, installMode: mode,
      ref, username, architecture, ubuntuVersion, moduleSelection: { profile } } };
  }, [ready, host, architecture, imageSupported, provider, mode, ref, username, ubuntuVersion, profile]);
  ...
  return <details className="rounded-xl border border-border/50 bg-card/30 p-4">
    ...
    <div className="mt-3 space-y-4">
      ...
      <p className="text-sm text-muted-foreground">
        Confirm the intended VPS image and architecture below. These are your declarations,
        not detected host facts. An older LTS image may still be upgraded by the installer.
      </p>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="space-y-1 text-sm" htmlFor={`${id}-image`}>
          <span>VPS Ubuntu image before installation</span>
          <select id={`${id}-image`} value={imageSupported ? ubuntuVersion : ""}
            disabled={!ready} onChange={(event) => setImageOverride(event.target.value)}
            className="min-h-11 w-full rounded-md border bg-background px-3">
            <option value="">Choose the VPS image</option>
            {VPS_UBUNTU_IMAGE_OPTIONS.map((version) => <option key={version} value={version}>Ubuntu {version}</option>)}
          </select>
        </label>
        <label className="space-y-1 text-sm" htmlFor={`${id}-arch`}>
          <span>VPS architecture</span>
          <select id={`${id}-arch`} value={architecture} disabled={!ready}
            onChange={(event) => {
              const value = event.target.value;
              if (value === "" || value === "x86_64" || value === "aarch64") setArchitecture(value);
            }} className="min-h-11 w-full rounded-md border bg-background px-3">
      ...
      {!context && <p role="status" className="text-sm text-muted-foreground">
        {!ready ? "Loading your saved wizard settings..."
          : !host ? "Record the target VPS address in the wizard first."
      ...
      <label className="block space-y-2 text-sm" htmlFor={`${id}-file`}>
        ...
        <input id={`${id}-file`} type="file" accept=".json,application/json" disabled={!context || busy}
          onChange={(event) => {
      ...
      {busy && <p role="status" className="text-sm">Reading and validating the local profile...</p>}
      {error && <p role="alert" className="break-words text-sm text-destructive">{error}</p>}
      {(busy || review || error) && <Button type="button" variant="outline" onClick={clear}>Clear imported profile</Button>}
      {activeReview && <div className="space-y-4 border-t border-border/50 pt-4">
        <h2 className="text-lg font-semibold">Review {activeReview.diff.profile?.displayName}</h2>
        <p className="break-all font-mono text-xs">File SHA-256: {activeReview.sourceSha256}</p>
        <Changes title="Provider and access defaults (not applied)" changes={activeReview.diff.safeDefaults.changes} />
        <Changes title="Installer and module-selection changes" changes={activeReview.diff.installerCommand.changes} />
        <div className="space-y-2 text-sm">
          <h3 className="font-semibold">Required dependencies added by the resolver</h3>
          <p className="break-words font-mono text-xs">{activeReview.diff.dependencyClosure.join(", ") || "None."}</p>
          <h3 className="font-semibold">Explicitly skipped modules</h3>
          <p className="break-words font-mono text-xs">{activeReview.diff.skips.requested.join(", ") || "None."}</p>
          <h3 className="font-semibold">Required sign-ins / secret slots</h3>
          <p className="break-words font-mono text-xs">{activeReview.diff.secretSlots.required.join(", ") || "None declared."}</p>
          <h3 className="font-semibold">Optional sign-ins / secret slots</h3>
          <p className="break-words font-mono text-xs">{activeReview.diff.secretSlots.optional.join(", ") || "None declared."}</p>
          <p className="text-muted-foreground">Supply credentials only through the normal provider or CLI login. Never paste them into a profile.</p>
          {activeReview.diff.skips.warnings.map((warning, index) => <p key={index} className="text-muted-foreground">{warning}</p>)}
        </div>
        ...
        {approved && <div className="space-y-2">
          ...
          <CommandCard command={approved.command} description="Approved team-profile installer" runLocation="vps" />
        </div>}
      </div>}
    </div>
  </details>;
