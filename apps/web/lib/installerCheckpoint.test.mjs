...
const api = load();
const input = () => ({
  command: 'bash install.sh --profile "cloud-only" --mode safe --ref "reviewed-tag"',
  host: "203.0.113.42",
  manifestSha256: "a".repeat(64),
  checksumsYamlSha256: "b".repeat(64),
});
const expected = (value) =>
  "run-flywheel-installer-v2-" +
  createHash("sha256")
    .update(
      JSON.stringify([
...
test("native Web Crypto matches the independently computed SHA-256 and produces an opaque key", async () => {
  const value = input();
  const checkpoint = await api.createInstallerCheckpoint(value);
...
test("identical context is stable across calls and property enumeration order", async () => {
  const value = input();
  const a = await api.createInstallerCheckpoint(value);
  const b = await api.createInstallerCheckpoint(
...
for (const [field, replacement] of [
  ["command", 'bash install.sh --profile "full" --mode vibe'],
  ["host", "203.0.113.43"],
  ["manifestSha256", "c".repeat(64)],
  ["checksumsYamlSha256", "d".repeat(64)],
]) {
  test(`binds ${field} into both the digest and late-result check`, async () => {
    const value = input();
    const old = await api.createInstallerCheckpoint(value);
    const next = { ...value, [field]: replacement };
    const changed = await api.createInstallerCheckpoint(next);
...
test("captures immutable context before awaiting the digest", async () => {
  ...
  const deferred = load({
    subtle: {
      digest: async (algorithm, bytes) => {
        captured = await webcrypto.subtle.digest(algorithm, bytes);
        await pending;
        return captured;
      },
    },
  ...
  const value = input();
  const before = { ...value };
  const promise = deferred.createInstallerCheckpoint(value);
  value.host = "203.0.113.99";
  value.command = "different-command";
  value.manifestSha256 = "f".repeat(64);
  resolve();
  const result = await promise;
  assert.equal(result.persistKey, expected(before));
  assert.equal(result.host, before.host);
  assert.equal(result.command, before.command);
...
test("JSON framing prevents ambiguity between command and host components", async () => {
  ...
  assert.notEqual(
    (await api.createInstallerCheckpoint(a)).persistKey,
    (await api.createInstallerCheckpoint(b)).persistKey,
  );
});
...
for (const [label, value] of [
  ...
  test(`refuses ${label} without exposing input values`, async () => {
    await assert.rejects(api.createInstallerCheckpoint(value), (error) => {
      ...
      assert.doesNotMatch(error.message, /SECRET|private|203\.0\.113/);
      return true;
    });
  });
}
...
test("fails closed without browser crypto instead of falling back to a shared completion key", async () => {
  ...
  await assert.rejects(
    noCrypto.createInstallerCheckpoint(input()),
...
test("missing, unbound, and malformed checkpoint results never match", async () => {
  const value = input();
  const valid = await api.createInstallerCheckpoint(value);
  ...
  assert.equal(
    api.installerCheckpointMatches({ ...valid, persistKey: "run-flywheel-installer" }, value),
    false,
  );
