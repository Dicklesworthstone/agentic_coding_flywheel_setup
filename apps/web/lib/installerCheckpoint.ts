...
export async function createInstallerCheckpoint(
  ...
  if (
    ...
    !input.host.trim() ||
    input.host.length > 256 ||
    /[\x00-\x20\x7f]/.test(input.host) ||
    typeof input.manifestSha256 !== "string" ||
    !/^[a-f0-9]{64}$/.test(input.manifestSha256) ||
    typeof input.checksumsYamlSha256 !== "string" ||
    !/^[a-f0-9]{64}$/.test(input.checksumsYamlSha256)
  ) {
...
export function installerCheckpointMatches(
  ...
  return Boolean(
    checkpoint &&
      input &&
      checkpoint.command === input.command &&
      checkpoint.host === input.host &&
...
export async function createDoctorCheckpoint(
  input: DoctorCheckpointInput,
): Promise<DoctorCheckpoint> {
  if (
    !input ||
    typeof input.doctorCommand !== "string" ||
    !input.doctorCommand.trim() ||
    input.doctorCommand.length > 65_536 ||
    input.doctorCommand.includes("\0")
  ) {
  ...
  const bytes = new TextEncoder().encode(
    JSON.stringify([
      "acfs.doctor-acknowledgement.v1",
      installation.persistKey,
      snapshot.doctorCommand,
    ]),
  );
  const digest = await globalThis.crypto.subtle.digest("SHA-256", bytes);
  const hex = Array.from(new Uint8Array(digest), (value) =>
  ...
  return Object.freeze({
    ...
    checksumsYamlSha256: installation.checksumsYamlSha256,
    doctorCommand: snapshot.doctorCommand,
    persistKey: `${DOCTOR_KEY_PREFIX}${hex}`,
  });
}
...
export function isRenderedCheckpointComplete(
  ...
  root: Pick<Document, "querySelectorAll"> | undefined = typeof document === "undefined"
    ? undefined
    : document,
): boolean {
  ...
  try {
    ...
    if (
      !new RegExp(`^${prefix}[a-f0-9]{64}$`).test(
        control.getAttribute("data-acfs-completion-key") ?? "",
      ) ||
