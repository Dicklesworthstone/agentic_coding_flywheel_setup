...
function commandSelection(command: string): ModuleSelectionInput {
  ...
  const args = execFileSync("/bin/bash", ["-c", prefix + command])
    .toString()
    .split("\0")
    .filter(Boolean);
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (arg === "--only") result.onlyModules!.push(args[++index]!);
      ...
      assert.ok(arg === "--yes" || arg === "--target-ubuntu=26.04", `Unexpected argument: ${arg}`);
  }
  return result;
}
...
for (const profile of manifestSelectionProfiles) {
  test(`team profile ${profile.id} preserves every accepted tag/category exclusion through real Bash arguments`, () => {
    ...
    for (const selection of groups) {
      ...
      assert.deepEqual(
        exported.install.modulePlan.included,
        original.included.map((entry) => entry.id),
      );
      assert.deepEqual(
        ids(commandSelection(imported.installerCommand.command!)),
        exported.install.modulePlan.included,
      );
      assert.deepEqual(
        ids(commandSelection(buildHandoffRunbook(inputs(selection)).install.command)),
        exported.install.modulePlan.included,
      );
    }
...
test("overlapping group and exact skips are deduplicated, including disabled defaults", () => {
  const selection: ModuleSelectionInput = {
    onlyModules: ["agents.claude"],
    skipCategories: ["cloud"],
    skipTags: ["cloud"],
    skipModules: ["cloud.wrangler", "cloud.wrangler"],
  };
  const profile = buildTeamProfile(inputs(selection));
  const expected = manifestModules
    .filter((module) => module.category === "cloud" || module.tags.includes("cloud"))
    .map((module) => module.id)
    .sort();
  assert.deepEqual([...profile.install.modules.skip].sort(), expected);
  assert.deepEqual(
    ids(commandSelection(buildTeamProfileImportDiff(profile).installerCommand.command!)),
    ids(selection),
  );
});
...
test("current group exclusions and imported exact skips compare without a false selection change", () => {
  ...
  assert.ok(
    !diff.installerCommand.changes.some((entry) => entry.field.startsWith("install.modules")),
  );
  const cleared = buildTeamProfileImportDiff(profile, { moduleSelection: {} });
  assert.ok(
    cleared.installerCommand.changes.some((entry) => entry.field === "install.modules.skip"),
  );
});

test("lowering does not mutate or alias input selections", () => {
  const selection: ModuleSelectionInput = {
    onlyModules: ["agents.claude", "agents.claude"],
    onlyPhases: [],
    skipTags: ["maintenance"],
    skipModules: [],
  };
...
const invalidSelections: unknown[] = [
  ...
  { onlyModules: [""] },
  { onlyModules: "agents.claude" },
  { onlyModules: new Array(1) },
  { skipTags: ["not-a-real-tag"] },
  { skipCategories: ["not-a-category"] },
  { onlyModule: ["agents.claude"] },
...
for (const [index, value] of invalidSelections.entries()) {
  test(`invalid selection ${index} cannot be exported or compared as a valid full install`, () => {
    ...
    const diff = buildTeamProfileImportDiff(buildTeamProfile(inputs()), {
      moduleSelection: value as ModuleSelectionInput,
    });
...
test("accessors are not evaluated while validating an exported selection", () => {
  let accesses = 0;
  const selection = Object.defineProperty({}, "skipTags", {
    enumerable: true,
    get() {
      accesses++;
      return ["maintenance"];
...
test("ordinary default and explicit-selector exports retain matching commands", () => {
  for (const selection of [
    {},
    { onlyModules: ["agents.claude"] },
    { profile: "cloud-only" as const },
  ]) {
    const profile = buildTeamProfile(inputs(selection));
    const diff = buildTeamProfileImportDiff(profile);
    assert.equal(diff.ok, true, JSON.stringify(diff.findings));
    assert.deepEqual(
      ids(commandSelection(diff.installerCommand.command!)),
      ids(commandSelection(buildInstallCommand("safe", "v1.2.3", "dev-user", selection))),
    );
  }
});
...
test("known session-related module IDs are public selector data, not a credential exemption for unknown values", () => {
  ...
  assert.equal(diff.installerCommand.command, null);
  assert.ok(
    diff.findings.some((finding) => finding.code === "team_profile_secret_material_refused"),
  );
});
