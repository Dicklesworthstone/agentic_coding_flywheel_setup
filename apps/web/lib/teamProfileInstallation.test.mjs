...
function fixture() {
  ...
  const context = { targetHost: '203.0.113.7', current: {
    architecture: 'aarch64', ubuntuVersion: '24.04', username: 'ubuntu', installMode: 'vibe',
    ref: null, moduleSelection: { profile: 'full' }, providerSelection: { providerId: 'other' },
  } };
  const profile = { profileId: 'team-example', displayName: 'Team Example',
    providerDefaults: { sshUser: 'team-user', architecture: 'aarch64', operatingSystem: 'ubuntu-26.04' },
  ...
  let valid = true; let revision = ''; let validations = 0;
  ...
  runInNewContext(compiled.outputText, { module, exports: module.exports, TextDecoder, crypto: webcrypto,
    structuredClone, require: (name) => {
      if (name === './generated/manifest-modules') return { manifestProvenance: metadata };
      assert.equal(name, './commandBuilder');
      return { buildTeamProfileImportDiff(input) {
        validations++;
        return { schema: 'acfs.team-profile-import-diff.v1', dryRun: true, ok: valid,
          profile: { profileId: input.profileId, displayName: input.displayName },
          findings: valid ? [] : [{ code: 'team_profile_manifest_mismatch' }],
          installerCommand: { command: `canonical-${input.install.mode}-${input.install.ref.value}${revision}`, changes: [] },
  ...
  const entries = new Map([['other-preference', 'preserve']]); const calls = [];
  const storage = {
    getItem(key) { calls.push(['get', key]); return entries.get(key) ?? null; },
    setItem(key, value) { calls.push(['set', key, value]); entries.set(key, value); },
    removeItem(key) { calls.push(['remove', key]); entries.delete(key); },
  };
  return { api, profile, context, storage, entries, calls, metadata,
    refuse: () => { valid = false; }, changePlan: () => { revision = '-changed'; }, validations: () => validations,
...
test('activation uses one immutable source-derived settings snapshot without importing provider claims', async () => {
  const f = fixture(); const review = await f.review();
  ...
  assert.equal(active.mode, 'safe'); assert.equal(active.username, 'team-user'); assert.equal(active.ref, 'v1.2.3');
  assert.equal(active.architecture, 'aarch64'); assert.equal(active.ubuntuVersion, '24.04');
  assert.deepEqual(clone(active.moduleSelection), { profile: 'full', onlyModules: ['agents.claude'],
    onlyPhases: [], skipModules: ['acfs.nightly'], noDeps: false });
  assert.equal(JSON.stringify([f.profile, f.context]), before);
  assert.ok(f.api.teamProfileInstallationMatches(active, f.context));
  assert.throws(() => active.moduleSelection.skipModules.push('other'), (error) => error.name === 'TypeError');
  assert.throws(() => { active.username = 'different'; }, TypeError);
...
test('only the reload guard is persisted; no profile, source digest, host, command, or credentials', async () => {
  ...
  assert.equal(f.api.teamProfileInstallationMatches(JSON.parse(JSON.stringify(review)), f.context), false);
});

test('main is kept as an unpinned command ref, avoiding a changed approved command', async () => {
  const f = fixture(); f.profile.install.ref.value = 'main';
  const active = f.api.activateTeamProfileInstallation(await f.review(), f.context, true, f.storage);
  assert.equal(active.ref, null); assert.equal(active.command, 'canonical-safe-main');
});

for (const mutation of ['host', 'image', 'architecture', 'username', 'mode', 'ref', 'selection', 'provider', 'manifest', 'checksums']) {
  test(`changed ${mutation} invalidates activation before any storage mutation`, async () => {
    ...
    assert.throws(() => f.api.activateTeamProfileInstallation(review, f.context, true, f.storage), /changed/);
...
test('rejects deserialized reviews and every non-true confirmation', async () => {
  ...
  for (const value of [false, undefined, 'true', 1, null]) {
    assert.throws(() => f.api.activateTeamProfileInstallation(review, f.context, value, f.storage), /approve/);
  }
  assert.throws(() => f.api.activateTeamProfileInstallation(clone(review), f.context, true, f.storage), /changed/);
  assert.equal(f.calls.length, 0);
});
...
for (const condition of ['refused', 'changed']) {
  test(`reruns canonical validation and refuses ${condition} plans before publishing settings`, async () => {
    ...
    assert.equal(f.validations(), 2); assert.equal(f.calls.length, 0);
  });
}
...
for (const condition of ['throw-write', 'silent-write', 'throw-read']) {
  test(`unavailable session storage cannot create an active installation: ${condition}`, async () => {
    const f = fixture(); const review = await f.review();
    if (condition === 'throw-write') f.storage.setItem = () => { throw new Error('PRIVATE'); };
    if (condition === 'silent-write') f.storage.setItem = () => {};
    if (condition === 'throw-read') f.storage.getItem = () => { throw new Error('PRIVATE'); };
    assert.throws(() => f.api.activateTeamProfileInstallation(review, f.context, true, f.storage),
      (error) => error.code === 'team_profile_session_unavailable' && !error.message.includes('PRIVATE'));
    // The existing manual approval path remains available without session adoption.
    assert.equal(f.api.approveTeamProfileReview(review, f.context, true).command, 'canonical-safe-v1.2.3');
  });
}

test('explicit discard touches only the marker and verifies its removal', () => {
  const f = fixture(); f.entries.set(f.api.TEAM_PROFILE_SESSION_KEY, 'review-required');
  f.api.discardTeamProfileInstallation(f.storage);
  assert.equal(f.api.readTeamProfileSessionGuard(f.storage), 'clear');
  assert.deepEqual([...f.entries], [['other-preference', 'preserve']]);
  f.entries.set(f.api.TEAM_PROFILE_SESSION_KEY, 'review-required'); f.storage.removeItem = () => {};
...
test('unknown nonempty/empty markers block restoration rather than inventing stored approval', () => {
  ...
  assert.equal(f.api.readTeamProfileSessionGuard({ getItem() { throw new Error(); } }), 'unavailable');
});
