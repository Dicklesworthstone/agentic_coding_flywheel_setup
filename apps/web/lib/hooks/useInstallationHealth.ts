...
import { commandCompletionKeys, COMMAND_COMPLETION_CHANGED_EVENT } from "@/components/command-card";
...
import {
  createDoctorCheckpoint, doctorCheckpointMatches,
  type DoctorCheckpoint, type DoctorCheckpointInput,
...
export function useInstallationHealth() {
  ...
  const ready = vpsIPLoaded && sshUsernameLoaded && installModeLoaded && acfsRefLoaded && moduleSelectionLoaded
    && (!session || session.status === "saved" || session.status === "active");
  const selectedPlan = useMemo(() => ready ? resolveModuleSelection(moduleSelection) : null,
    [ready, moduleSelection]);
  const reinstallCommand = useMemo(() => {
    if (!ready || !selectedPlan?.ok || !isValidIP(vpsIP ?? "")
        || normalizeSSHUsername(sshUsername) !== sshUsername || !["safe", "vibe"].includes(installMode)
        || (acfsRef !== null && normalizeGitRef(acfsRef) !== acfsRef)) return null;
    try {
      const command = buildInstallCommand(installMode, acfsRef, sshUsername, moduleSelection);
      if (session?.status === "active" && (!session.installation || command !== session.installation.command)) return null;
      return command;
    } catch { return null; }
  }, [ready, selectedPlan, vpsIP, sshUsername, installMode, acfsRef, moduleSelection, session]);
  const checkpointInput = useMemo<DoctorCheckpointInput | null>(() => reinstallCommand && vpsIP ? {
    command: reinstallCommand, doctorCommand: DOCTOR_COMMAND, host: vpsIP, manifestSha256, checksumsYamlSha256,
  } : null, [reinstallCommand, vpsIP, manifestSha256, checksumsYamlSha256]);
  ...
  useEffect(() => {
    let current = true;
    if (checkpointInput) {
      void createDoctorCheckpoint(checkpointInput).then((value) => {
        if (current) { setCheckpoint(value); setHashFailure(null); }
      }).catch(() => { if (current) setHashFailure(checkpointInput); });
    }
  ...
  const { data: acknowledged = false, status: acknowledgementStatus } = useQuery({
    queryKey: completionKey ? commandCompletionKeys.completion(completionKey) : ["doctor-checkpoint-pending"],
    queryFn: () => completionKey ? safeGetItem(completionKey) === "true" : false,
    enabled: completionKey !== null, staleTime: Infinity, gcTime: Infinity,
  });
  ...
  const doctorConfirmed = Boolean(activeCheckpoint) && acknowledgementStatus === "success" && acknowledged === true;
  const loading = !ready || Boolean(checkpointInput && !activeCheckpoint && !hashFailed)
    || Boolean(activeCheckpoint && acknowledgementStatus === "pending");
  return { ready, loading, vpsIP, sshUsername, selectedPlan, reinstallCommand, activeCheckpoint,
    hashFailed, completionKey, doctorConfirmed };
}
