...
function fixture() {
  const calls = [];
  let reject = false;
  let command = 'bash install.sh --profile "cloud-only" --skip "acfs.nightly"';
    ...
    "./commandBuilder": {
      buildTeamProfileImportDiff(source, current) {
        ...
        return {
          ...
          installerCommand: {
            command: reject ? null : command,
            changes: [{ field: "install.profile", current: "full", next: "cloud-only" }],
          },
          dependencyClosure: ["lang.bun"],
          skips: { requested: ["acfs.nightly"], allowed: !reject, warnings: [] },
          secretSlots: { required: ["secret://acfs/team/github-auth"], optional: [] },
          incompatibilities: [],
          refusals: [],
        };
      },
...
test("reads real Blob bytes, binds independent SHA-256, and withholds command until confirmation", async () => {
  const item = fixture();
  const text = '{ "schema": "fixture", "name":"日本語" }';
  ...
  assert.equal(Object.isFrozen(review.diff.installerCommand), true);
...
for (const text of [
  ...
  test(`rejects ambiguous or invalid JSON ${JSON.stringify(text)}`, async () => {
    ...
    await assert.rejects(item.api.reviewTeamProfileFile(file(text), item.context), (error) => {
      assert.equal(error.code, "team_profile_file_invalid");
...
test("accepts escaped strings, harmless repeated names in different objects and standard whitespace", async () => {
  const item = fixture();
  const value = { 'a"\\b': ["x", { same: 1 }, { same: 2 }], punctuation: "{},[]" };
...
test("enforces depth and node budgets before calling canonical validation", async () => {
  const item = fixture();
  for (const text of [
    '{"a":' + "[".repeat(34) + "0" + "]".repeat(34) + "}",
    '{"a":' + JSON.stringify(Array(16_385).fill(0)) + "}",
  ]) {
    await assert.rejects(
      item.api.reviewTeamProfileFile(file(text), item.context),
      code("team_profile_file_invalid"),
    );
  }
  assert.equal(item.calls.length, 0);
});
...
test("bounds file reads before allocation and enforces exact byte-count snapshots", async () => {
  ...
  for (const size of [0, -1, NaN, 1.5, item.api.TEAM_PROFILE_FILE_LIMIT + 1]) {
    await assert.rejects(
      item.api.reviewTeamProfileFile(
        {
          size,
          slice() {
            reads++;
...
test("rejects invalid UTF-8 and redacts native read failures", async () => {
  ...
  await assert.rejects(
    item.api.reviewTeamProfileFile(
      new Blob([Buffer.from([123, 34, 0xff, 34, 58, 49, 125])]),
      item.context,
    ),
    code("team_profile_file_invalid"),
  );
  await assert.rejects(
    ...
    (error) => {
      assert.equal(error.code, "team_profile_read_failed");
...
for (const confirmed of [false, undefined, 1, "true"]) {
  test(`requires actual explicit confirmation, not ${JSON.stringify(confirmed)}`, async () => {
    const item = fixture();
    const review = await item.api.reviewTeamProfileFile(file(), item.context);
    assert.throws(
      () => item.api.approveTeamProfileReview(review, item.context, confirmed),
      code("team_profile_confirmation_required"),
    );
...
for (const [name, change] of [
  [
    "host",
    (c) => {
      c.targetHost = "203.0.113.43";
    },
  ],
  [
    "architecture",
    (c) => {
      c.current.architecture = "aarch64";
    },
  ],
  [
    "image",
    (c) => {
      c.current.ubuntuVersion = "24.04";
    },
  ],
  [
    "mode",
    (c) => {
      c.current.installMode = "vibe";
    },
  ],
  [
    "ref",
    (c) => {
      c.current.ref = "another";
    },
  ],
  [
    "username",
    (c) => {
      c.current.username = "developer";
    },
  ],
  [
    "selection",
    (c) => {
      c.current.moduleSelection = { onlyModules: ["agents.claude"] };
    },
  ],
]) {
  test(`changed ${name} invalidates approval without reusing a retained command`, async () => {
    ...
    assert.throws(
      () => item.api.approveTeamProfileReview(review, item.context, true),
      code("team_profile_review_changed"),
    );
  });
}
...
test("changed canonical provenance and changed validator output both invalidate approval", async () => {
  ...
  const review2 = await second.api.reviewTeamProfileFile(file(), second.context);
  second.changeCommand();
  assert.throws(
    () => second.api.approveTeamProfileReview(review2, second.context, true),
    code("team_profile_review_changed"),
  );
  second.reject();
  assert.throws(
    () => second.api.approveTeamProfileReview(review2, second.context, true),
    code("team_profile_review_blocked"),
  );
});
...
test("deserialized or fabricated reviews cannot authorize a command", async () => {
  ...
  for (const copy of [{}, structuredClone(review), { ...review }]) {
    ...
    assert.throws(
      () => item.api.approveTeamProfileReview(copy, item.context, true),
      code("team_profile_review_changed"),
    );
  }
...
test("context is captured before asynchronous file reads and cannot be reassociated afterwards", async () => {
  ...
  const pending = item.api.reviewTeamProfileFile(
    {
      size: 2,
      slice() {
        return {
  ...
  item.context.current.username = "another-user";
  resume();
...
test("missing secure crypto cannot mint a review", async () => {
  ...
  await assert.rejects(
    item.api.reviewTeamProfileFile(file(), item.context),
    code("team_profile_hash_unavailable"),
  );
  assert.equal(item.calls.length, 0);
});
...
test("explicit complete target context is required before reading", async () => {
  ...
  for (const current of [
    ...
    { architecture: "unknown", ubuntuVersion: "26.04" },
  ]) {
    await assert.rejects(
      item.api.reviewTeamProfileFile(file(), { ...item.context, current }),
      code("team_profile_context_required"),
    );
  }
  await assert.rejects(
    item.api.reviewTeamProfileFile(file(), { ...item.context, targetHost: "" }),
    code("team_profile_context_required"),
  );
});
