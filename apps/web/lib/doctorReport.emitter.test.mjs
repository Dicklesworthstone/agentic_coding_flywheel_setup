...
const source = readFileSync(
  process.env.ACFS_DOCTOR_TEST_SOURCE ?? new URL("../../../scripts/lib/doctor.sh", import.meta.url),
  "utf8",
);
...
for (const deep of [false, true]) {
  test(`accepts actual print_json output with mixed check results (deep=${deep})`, async () => {
    const checks = [
      ...
      { id: "agent.codex", status: "skip" },
      { id: "deep.cloud.gh_auth", status: "timeout" },
    ...
    assert.equal(raw.deep_mode, deep);
    assert.equal(raw.summary.warn, 2);
    assert.equal(raw.deep_summary !== undefined, deep);
    const value = await reader.reviewDoctorReportFile(new Blob([bytes]), {
      host: "203.0.113.7",
      username: "fixture-user",
      mode: "safe",
      installerCommand: "fixture-installer",
      manifestSha256: "a".repeat(64),
      checksumsYamlSha256: "b".repeat(64),
      selectedModuleIds: ["lang.bun", "agents.claude"],
      knownModuleIds: ["lang.bun", "agents.claude", "agents.codex"],
    });
    assert.equal(value.modules[0].status, "pass");
    assert.equal(value.modules[1].status, "fail");
    assert.equal(value.unmapped.timeout, 1);
    assert.equal(value.unmapped.warn, 1);
    assert.equal(value.outsideSelection.skip, 1);
    ...
    assert.ok(!JSON.stringify(value).includes("do-not-run"));
  });
}
