...
function fixture(crypto = webcrypto) {
  ...
  const output = ts.transpileModule(readFileSync(new URL('./installerCheckpoint.ts', import.meta.url), 'utf8'), {
  ...
  runInNewContext(output.outputText, { module, exports: module.exports, crypto, TextEncoder }, { timeout: 5000 });
  return module.exports;
}
const input = () => ({ command: 'bash install.sh --only agents.claude', doctorCommand: 'acfs doctor',
...
test('doctor acknowledgement binds both commands and is distinct from installation acknowledgement', async () => {
  const api = fixture(); const source = input();
...
for (const field of ['command', 'doctorCommand', 'host', 'manifestSha256', 'checksumsYamlSha256']) {
  test(`changing ${field} requires a new doctor acknowledgement`, async () => {
    const api = fixture(); const source = input(); const checkpoint = await api.createDoctorCheckpoint(source);
    source[field] = field.endsWith('Sha256') ? 'c'.repeat(64) : field === 'host' ? '203.0.113.99' : `${source[field]} --changed`;
...
for (const value of ['', ' ', null, 1, 'a\0b', 'x'.repeat(65_537)]) {
  test(`refuses malformed doctor command (${typeof value}, ${String(value).length} characters)`, async () => {
    const api = fixture(); await assert.rejects(api.createDoctorCheckpoint({ ...input(), doctorCommand: value }));
  });
}

test('validates the complete installation context, not only the doctor command', async () => {
  const api = fixture();
  for (const change of [{ command: '' }, { host: '' }, { host: 'bad host' }, { manifestSha256: 'bad' }, { checksumsYamlSha256: '' }]) {
...
test('captures the complete input before either asynchronous digest', async () => {
  let resume; const wait = new Promise((done) => { resume = done; }); let calls = 0;
  const api = fixture({ subtle: { async digest(...args) { calls++; await wait; return webcrypto.subtle.digest(...args); } } });
  const source = input(); const original = { ...source }; const pending = api.createDoctorCheckpoint(source);
...
test('unavailable or failed secure hashing cannot invent a shared fallback key', async () => {
  for (const crypto of [null, {}, { subtle: { digest: async () => { throw new Error('unavailable'); } } }]) {
...
function control(key = `flywheel-doctor-v2-${'a'.repeat(64)}`, attributes = {}, extra = {}) {
  ...
  return { tagName: 'BUTTON', getAttribute: (name) => attrs[name] ?? null,
    hasAttribute: (name) => Object.hasOwn(attrs, name), closest: () => null, ...extra };
}
function root(controls) { return { querySelectorAll: () => controls }; }
...
test('shared navigation requires exactly one currently rendered acknowledged scoped control', () => {
  ...
  assert.equal(api.isRenderedCheckpointComplete('installer', root([control(`run-flywheel-installer-v2-${'b'.repeat(64)}`)])), true);
});

for (const key of ['flywheel-doctor', 'run-flywheel-installer', 'flywheel-doctor-v2-invalid', `flywheel-doctor-v2-${'a'.repeat(65)}`]) {
...
for (const state of [ { 'data-state': 'unchecked' }, { 'data-state': 'indeterminate' }, { disabled: '' },
  { 'data-disabled': '' }, { 'aria-disabled': 'true' } ]) {
  test(`refuses unchecked, unhydrated or disabled control ${JSON.stringify(state)}`, () => {
    assert.equal(fixture().isRenderedCheckpointComplete('doctor', root([control(undefined, state)])), false);
  });
}

test('native checkboxes work; hidden/inert controls and failed DOM reads fail closed', () => {
  const api = fixture();
  const native = control(undefined, { 'data-state': null }, { tagName: 'INPUT', type: 'checkbox', checked: true });
  assert.equal(api.isRenderedCheckpointComplete('doctor', root([native])), true);
  native.checked = false; assert.equal(api.isRenderedCheckpointComplete('doctor', root([native])), false);
  assert.equal(api.isRenderedCheckpointComplete('doctor', root([control(undefined, {}, { closest: () => ({ hidden: true }) })])), false);
  assert.equal(api.isRenderedCheckpointComplete('doctor', { querySelectorAll() { throw new Error('unavailable'); } }), false);
