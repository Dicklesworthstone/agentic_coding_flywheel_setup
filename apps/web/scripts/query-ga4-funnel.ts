...
async function queryLessonFunnel(): Promise<FunnelStep[]> {
  ...
  try {
    ...
    if (steps.length === 0) {
      console.log(
        "   Note: No lesson tracking events detected yet - learning hub may need traffic.\n",
      );
    }
...
async function queryOverviewMetrics(): Promise<void> {
  ...
  try {
    ...
    if (response.rows && response.rows[0]) {
      ...
      console.log(
        `  Avg Session:      ${Math.floor(avgDuration / 60)}m ${Math.floor(avgDuration % 60)}s`,
      );
...
function printFunnelChart(
  ...
  if (firstStep && lastStep) {
    const overallConversion =
      firstStep.users > 0 ? ((lastStep.users / firstStep.users) * 100).toFixed(1) : "0";
    console.log("  ─".repeat(35));
    console.log(
      `  Overall Conversion: ${overallConversion}% (${lastStep.users}/${firstStep.users} users)`,
    );
  }
}
