...
export const ALLOWED_LESSON_COMPLETION_PERCENTAGES = new Set(
  Array.from({ length: TOTAL_LESSONS }, (_, index) =>
    Math.round(((index + 1) / TOTAL_LESSONS) * 100),
  ),
);
...
export function isServerConversionType(value: unknown): value is ServerConversionType {
  return typeof value === "string" && Object.hasOwn(SERVER_CONVERSION_VALUES, value);
}

export function serverEventParamsArePrivacySafe(eventName: string, params: unknown): boolean {
  ...
  const scalarValuesAreSafe = entries.every(([key, value]) => {
    ...
    if (typeof value === "string") {
      return (
        value.length <= MAX_PARAM_STRING_LENGTH && analyticsPayloadIsPrivacySafe({ [key]: value })
      );
    }
    if (typeof value === "number") {
      return Number.isFinite(value) && analyticsPayloadIsPrivacySafe({ [key]: value });
    }
    return typeof value === "boolean" && analyticsPayloadIsPrivacySafe({ [key]: value });
  });
  ...
  switch (eventName) {
    case "conversion": {
      ...
      return (
        isServerConversionType(conversionType) &&
        params.conversion_value === SERVER_CONVERSION_VALUES[conversionType]
      );
    }
    case "lesson_complete": {
      if (!hasExactlyKeys(params, ["lesson_id", "lesson_slug", "completion_percentage"]))
        return false;
