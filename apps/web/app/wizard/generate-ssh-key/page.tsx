...
import { type OperatingSystem, useUserOS } from "@/lib/userPreferences";
...
const OPEN_TERMINAL_INSTRUCTIONS: Record<OperatingSystem, ReactNode> = {
  mac: (
    <>
      Open the terminal app you installed (Ghostty or WezTerm). You can press{" "}
      <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">⌘</kbd> +{" "}
      <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">Space</kbd>, type the name,
      and press Enter.
    </>
  ),
  linux: (
    <>
      Open your terminal emulator. On most Linux distributions, press{" "}
      ...
      <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">T</kbd>, or find Terminal in
      your applications menu.
    </>
  ),
  windows: (
    <>
      Open Windows Terminal. Click Start, type &quot;Terminal&quot;, and click on Windows Terminal.
    </>
  ),
};
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-2">
        ...
        <p className="text-muted-foreground">
          This is your secure &quot;login key&quot; for connecting to your{" "}
          <Jargon term="vps">VPS</Jargon>.
        </p>
      </div>

      {/* Explanation */}
      <AlertCard variant="info" title="How SSH keys work">
        You&apos;re creating a <strong className="text-foreground">key pair</strong>: a{" "}
        <Jargon term="private-key">private key</Jargon> (stays on your computer) and a{" "}
        <Jargon term="public-key">public key</Jargon> (safe to share when setting up server login).
      ...
      <AlertCard variant="tip" title="Where are SSH keys stored?">
        <p className="mb-2">
          SSH keys are stored in a special folder called{" "}
          <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">~/.ssh</code> on your
          computer.
        </p>
        <ul className="space-y-1 text-sm">
          ...
          <li className="text-muted-foreground">
            The <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">~</code> symbol is
            shorthand for your home folder.
          </li>
          <li className="text-muted-foreground">
            The <code className="rounded bg-muted px-1 py-0.5 font-mono text-xs">.</code> prefix
            makes it a hidden folder (that&apos;s normal for config files).
          </li>
        </ul>
      </AlertCard>
      ...
      <div className="flex gap-3 rounded-xl border border-green/25 bg-green/5 p-3 sm:p-4">
        ...
        <div className="min-w-0 space-y-1">
          ...
          <p className="text-[12px] leading-relaxed text-muted-foreground sm:text-[13px]">
            These commands run{" "}
            <strong className="text-foreground/80">entirely on your machine</strong>. This website
            cannot see, access, or store your SSH keys. We&apos;re just showing you what to type.
            The{" "}
            <a
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          This creates the folder where your key files will be saved. On Windows, PowerShell only
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-lg border border-border/50 bg-muted/30 p-3 text-sm">
          <div className="space-y-2">
            <p>
              <strong className="text-foreground">If the key is missing:</strong>{" "}
              <span className="text-muted-foreground">
                it creates a new Ed25519 key with an empty passphrase.
              </span>
            </p>
            <p>
              <strong className="text-foreground">If the key already exists:</strong>{" "}
              <span className="text-muted-foreground">it does not overwrite your private key.</span>
            </p>
            <p>
              <strong className="text-foreground">No prompts expected:</strong>{" "}
              <span className="text-muted-foreground">
                you should not need to answer an overwrite or passphrase question.
              </span>
            </p>
          </div>
        </div>
      ...
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          ...
          <code className="rounded bg-muted px-1 py-0.5 text-xs">ssh-ed25519</code>.
        </p>
        <CommandCard
      ...
      <AlertCard variant="warning" title="Save your public key for later">
        Save this somewhere safe like a notes app. If your VPS was created with password-only
        access, you may need it for the follow-up SSH key command after installation. Do not paste
        the same public key into multiple setup steps; if that already happened, ACFS skips exact
        duplicate key lines when it copies server keys.
      </AlertCard>
      ...
      <DetailsSection summary="Having trouble? Click for common fixes">
        <div className="space-y-4 text-sm">
          ...
          <div>
            <p className="font-medium text-foreground">Key file already exists</p>
            <p className="text-muted-foreground">
              If you already have a key, you can use that one. Just copy the .pub file content.
            </p>
          </div>
        </div>
      </DetailsSection>
      ...
      <SimplerGuide>
        <div className="space-y-6">
          <GuideExplain term="an SSH Key">
            An SSH key is like a special password that lets you securely connect to another computer
            over the internet.
            <br />
            <br />
            Unlike regular passwords that you type, SSH keys are files stored on your computer.
            There are always <strong>two files</strong>:
            <br />
            <br />
            <strong>1. Private key:</strong> This is your secret key. It stays on YOUR computer and
            you never share it with anyone. It&apos;s like the key to your house.
            <br />
            <br />
            <strong>2. Public key:</strong> This is the one you share. You&apos;ll give this to your
            VPS provider. It&apos;s like giving someone a copy of your lock so they know it&apos;s
            really you when you connect.
          </GuideExplain>
          ...
          <GuideSection title="Detailed Step-by-Step Instructions">
            <div className="space-y-4">
              ...
              <GuideStep number={2} title="Copy the command">
                Look at the gray box above that shows the key command. Click the{" "}
                <strong>copy button</strong> (it looks like two overlapping squares) on the right
                side of the box. This copies the command to your clipboard.
              </GuideStep>

              <GuideStep number={3} title="Paste and run the command">
                Click inside the terminal window to make sure it&apos;s active. Then paste the
                command:
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  ...
                  <li>
                    <strong>Linux:</strong> Press{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">Ctrl</kbd> +{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">Shift</kbd> +{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">V</kbd>
                  </li>
                  <li>
                    <strong>Windows:</strong> Right-click inside the terminal OR press{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">Ctrl</kbd> +{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">V</kbd>
                  </li>
                </ul>
              ...
              <GuideStep number={4} title="Let the command finish">
                The command handles the file path and empty passphrase for you. It also checks for
                an existing ACFS key before creating anything new.
                <div className="mt-4 space-y-4">
                  <div>
                    <p className="mb-2 font-medium text-foreground">If this is your first run</p>
                    <OutputPreview title="You will see:">
                      <p className="text-green">
                        Your identification has been saved in /Users/you/.ssh/acfs_ed25519
                      </p>
                      <p className="text-green">
                        Your public key has been saved in /Users/you/.ssh/acfs_ed25519.pub
                      </p>
                    </OutputPreview>
                    <p className="mt-2 text-sm text-muted-foreground">
                      The new key was created without asking you where to save it or what passphrase
                      to use.
                    </p>
                    <div className="mt-2 rounded border border-border/30 bg-muted/20 p-2 text-xs text-muted-foreground">
                      <strong className="text-foreground">What this means:</strong> The path shown
                      (like{" "}
                      <code className="rounded bg-muted px-1 py-0.5 font-mono">
                        /Users/you/.ssh/acfs_ed25519
                      </code>
                      ) is where your key files will be saved. The{" "}
                      <code className="rounded bg-muted px-1 py-0.5 font-mono">.ssh</code> folder is
                      a standard location for SSH keys on all computers.
                    </div>
                  </div>

                  <div>
                    <p className="mb-2 font-medium text-foreground">
                      If you already ran this before
                    </p>
                    <OutputPreview title="You may see little or no output">
                      <p className="text-green">
                        The command rebuilds acfs_ed25519.pub from your existing private key.
                      </p>
                    </OutputPreview>
                    <p className="mt-2 text-sm text-muted-foreground">
                    ...
                    <div className="mt-2 rounded border border-border/30 bg-muted/20 p-2 text-xs text-muted-foreground">
                      <strong className="text-foreground">Why no passphrase?</strong> A passphrase
                      would add an extra password you&apos;d have to type every time you connect.
                      For a development VPS that you control, this extra security isn&apos;t
                      necessary and would slow you down. Your private key file itself is already
                      secure because it never leaves your computer.
                    </div>
                  </div>
                </div>
              </GuideStep>
...
+----[SHA256]-----+`}</pre>
                ...
                <p className="mt-3 text-sm">
                  <strong className="text-green">The randomart pattern means it worked!</strong> You
                  now have SSH keys.
                </p>
              </GuideStep>
            </div>
          ...
          <GuideSection title="Verify Your Key Was Created">
            <p className="mb-4">Let&apos;s make sure your keys were created correctly:</p>
            <div className="space-y-4">
              <GuideStep number={1} title="Check the files exist">
                ...
                />
                <p className="mt-3 text-sm text-muted-foreground">You should see two files:</p>
                <ul className="mt-2 list-disc space-y-1 pl-5 text-sm">
                  <li>
                    ...
                    <span className="text-muted-foreground">
                      {" "}
                      — Your <strong className="text-foreground">private key</strong> (keep this
                      secret!)
                    </span>
                  </li>
                  <li>
                    ...
                    <span className="text-muted-foreground">
                      {" "}
                      — Your <strong className="text-foreground">public key</strong> (this gets
                      shared)
                    </span>
                  </li>
                </ul>
            ...
            <GuideTip>
              Think of it like a mailbox: the <strong>public key</strong> is your address (you share
              it so people can send you mail), and the <strong>private key</strong>
          ...
          <GuideSection title="Now Copy Your Public Key">
            <div className="space-y-4">
              <GuideStep number={1} title="Run the second command">
                Now look at the second gray command box (the &quot;cat&quot; or &quot;type&quot;
                command). Click its copy button and paste it into the terminal, then press Enter.
              </GuideStep>
              ...
              <GuideStep number={2} title="Select and copy the output">
                ...
                <ul className="mt-2 list-disc space-y-1 pl-5">
                  <li>
                    <strong>Mac:</strong> Triple-click to select the whole line, then{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">⌘</kbd> +{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">C</kbd>
                  </li>
                  <li>
                    <strong>Linux:</strong> Triple-click to select the whole line, then{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">Ctrl</kbd> +{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">Shift</kbd> +{" "}
                    <kbd className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">C</kbd>
                  </li>
                  <li>
                    <strong>Windows:</strong> Triple-click to select, then right-click to copy
                  </li>
                </ul>
              </GuideStep>

              <GuideStep number={3} title="Save it somewhere safe">
                Open a notes app (like Notes on Mac, Notepad on Windows, or any text editor on
                Linux) and paste your public key there. You may need it later for the follow-up SSH
                key command after the installer finishes (not when creating the VPS—we&apos;ll use a
                password for that).
              </GuideStep>
            </div>
          </GuideSection>
          ...
          <GuideCaution>
            <strong>Never share your private key!</strong> The private key is the file WITHOUT
            &quot;.pub&quot; at the end. Only share the public key (the one that ends in
            &quot;.pub&quot;). If anyone asks for your private key, that&apos;s a scam.
          </GuideCaution>
          ...
          <GuideSection title="What if something went wrong?">
            <p>
              ...
              <strong>&quot;File already exists&quot;:</strong> Make sure you copied the full
              command from Step 2. The ACFS command checks for an existing private key first and
              should not ask to overwrite it.
            </p>
          </GuideSection>
        </div>
      </SimplerGuide>
