...
const compiled = ts.transpileModule(readFileSync(pageUrl, 'utf8'), {
  fileName: fileURLToPath(pageUrl), reportDiagnostics: true,
...
const profiles = [
  { id: 'full', label: 'Full' }, { id: 'safe', label: 'Safe', mode: 'safe' },
  { id: 'vibe', label: 'Vibe', mode: 'vibe' }, { id: 'minimal', label: 'Minimal' },
  { id: 'agents-only', label: 'Agents only' }, { id: 'cloud-only', label: 'Cloud only' },
...
function fixture(initial = {}) {
  const prefs = { os: 'linux', mode: 'safe', ref: null, ip: '203.0.113.42', username: 'developer',
    profile: 'cloud-only', provider: { providerId: 'other', planName: 'custom plan', ubuntuVersion: '24.04',
  ...
  let checkpointPending = false; let checkpointFailed = false; let checkpointOverride;
  ...
  const artifact = (kind) => (input) => {
    ...
    return { kind, input: plain(input), command: command(input.mode ?? input.installMode,
      input.ref ?? input.sourceRef ?? null, input.username, input.moduleSelection) };
  };
  const deps = {
    ...
    '@tanstack/react-query': { useQuery: (options) => {
      if (options.queryKey[0] === 'installerCheckpoint') {
        ...
        if (!input || checkpointPending || checkpointFailed) return { data: undefined, isError: checkpointFailed };
        const digest = createHash('sha256').update(JSON.stringify(['acfs.installer-acknowledgement.v1',
          input.command, input.host, input.manifestSha256, input.checksumsYamlSha256])).digest('hex');
      ...
      if (acknowledgeNext && options.enabled !== false) {
        acknowledgements.set(lastCompletionKey, true); acknowledgeNext = false;
      }
      return { data: acknowledgements.get(lastCompletionKey) ?? false };
    } },
    'next/navigation': { useRouter: () => ({ push: (path) => navigations.push(path), replace: () => {} }) },
    'lucide-react': Object.fromEntries(['Sparkles', 'Clock', 'ExternalLink', 'Check', 'Rocket',
      'ShieldCheck', 'Code', 'Wifi', 'Pin', 'Info', 'Download', 'FileJson', 'FileText'].map((name) => [name, `icon-${name}`])),
    '@/components/ui/button': { Button: 'button' },
    '@/components/ui/checkbox': { Checkbox: 'checkbox' },
    '@/components/command-card': { CommandCard: 'command-card', commandCompletionKeys: { completion: (key) => ['completion', key] } },
    '@/components/alert-card': { AlertCard: 'alert-card', OutputPreview: 'output-preview', DetailsSection: 'details-section' },
    ...
    '@/lib/wizardSteps': { canAccessWizardStep: () => true, getCompletedSteps: () => [],
      getNextReachableWizardStep: () => ({ slug: 'create-vps' }), markStepComplete: (step) => completed.push(step),
      useWizardForwardNav: (options) => { navigation = options; return { current: null }; } },
    '@/lib/hooks/useWizardAnalytics': { useWizardAnalytics: () => ({ markComplete: () => {} }) },
    '@/lib/utils': { copyTextToClipboard: async (value) => { copies.push(value); return true; },
      safeGetItem: () => null, withCurrentSearch: (path) => path },
    '@/lib/userPreferences': {
    ...
    '@/lib/commandBuilder': {
      ...
      buildInstallCommandDetails: (...args) => ({ command: command(...args), targetUbuntu: prefs.installDestination ?? '26.04' }),
      buildHandoffRunbook: artifact('runbook'), buildTeamProfile: artifact('profile'),
      formatHandoffRunbookMarkdown: JSON.stringify, serializeHandoffRunbookJson: JSON.stringify,
      formatTeamProfileReviewMarkdown: JSON.stringify, serializeTeamProfileJson: JSON.stringify,
      formatSshTarget: (username, host) => `${username}@${host}`,
    },
    '@/lib/providerProvisioningPacket': { buildProviderProvisioningPacket: artifact('provider'),
      serializeProviderProvisioningPacketJson: JSON.stringify },
  };
  const context = createContext({ setTimeout: () => 1, clearTimeout: () => {}, crypto: webcrypto, TextEncoder });
  const checkpointSource = ts.transpileModule(readFileSync(new URL('../../../lib/installerCheckpoint.ts', import.meta.url), 'utf8'), {
    compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 },
  });
  const checkpointModule = { exports: {} };
  runInContext(`(function(module,exports){${checkpointSource.outputText}\n})`, context)(checkpointModule, checkpointModule.exports);
  ...
  runInContext(`(function(require,module,exports){${compiled.outputText}\n})`, context,
    { filename: fileURLToPath(pageUrl), timeout: 5000 })((name) => {
  ...
  function render() {
    ...
    const find = (predicate) => { const node = nodes.find(predicate); assert.ok(node, 'Expected page control'); return node; };
    return {
      nodes,
      main: () => nodes.find((node) => node.type === 'command-card' && node.props.description === 'Agent Flywheel installer one-liner'),
      cache: () => nodes.find((node) => node.type === 'command-card' && node.props.description === 'Installer one-liner using the local cache'),
      ...
      pinInput: () => find((node) => node.type === 'input' && node.props['aria-label'] === 'Git ref to pin the installer to'),
  ...
  return { render, prefs, loaded, calls, copies, navigations, completed, provenance, acknowledgements,
    ...
    setCheckpointPending: (value) => { checkpointPending = value; },
    setCheckpointFailed: (value) => { checkpointFailed = value; },
    overrideCheckpoint: (value) => { checkpointOverride = value; },
    failPlan: () => { invalidPlan = true; } };
}

for (const { id } of profiles) {
  test(`saved ${id} profile reaches every executable and export path`, async () => {
    const item = fixture({ profile: id }); const view = item.render();
    ...
    for (const name of ['Runbook JSON', 'Runbook Markdown', 'Provider Packet', 'Team Profile', 'Profile Review']) {
...
test('changing the native profile picker refreshes memoized commands and all exports', async () => {
  const item = fixture(); const before = item.render();
  ...
  for (const kind of ['runbook', 'provider', 'profile']) assert.equal(item.calls[kind].at(-1).moduleSelection.profile, 'stack-only');
  after.button('Runbook JSON').props.onClick(); await Promise.resolve();
...
test('mode-bearing profiles cannot be downgraded by a conflicting saved install mode', () => {
  for (const mode of ['safe', 'vibe']) {
    const item = fixture({ profile: mode, mode: mode === 'safe' ? 'vibe' : 'safe' }); item.render();
...
test('saved profile hydration gates every command and artifact builder', () => {
  const item = fixture(); item.loaded.profile = false;
...
test('all other preference hydration gates remain intact', () => {
  for (const key of ['os', 'mode', 'ref', 'ip', 'username', 'provider']) {
    const item = fixture(); item.loaded[key] = false;
...
test('a missing host cannot generate commands or unlock the mobile continuation', () => {
  const item = fixture({ ip: null }); item.setAcknowledged(true);
  const view = item.render();
  assert.equal(view.main(), undefined); assert.equal(item.calls.command.length, 0);
...
test('invalid and blank pinned drafts block main, cache, exports, desktop and mobile continuation', async () => {
  for (const draft of ['bad;ref', '', '  ', '../escape']) {
    const item = fixture({ ref: 'reviewed-tag' }); item.setAcknowledged(true);
    item.render().pinInput().props.onChange({ target: { value: draft } });
    const count = item.calls.command.length; const view = item.render();
    assert.equal(view.main(), undefined, draft); assert.equal(view.cache(), undefined, draft);
    assert.equal(item.calls.command.length, count, 'invalid drafts must not reach builders');
    assert.match(view.errors(), /blocked/);
    for (const name of ['Runbook JSON', 'Runbook Markdown', 'Provider Packet', 'Team Profile', 'Profile Review']) {
      const button = view.button(name); assert.equal(button.props.disabled, true);
      button.props.onClick();
    }
    await Promise.resolve(); assert.deepEqual(item.copies, []);
    assert.equal(view.button('Installation finished').props.disabled, true);
    assert.equal(view.navigation().disabled, true);
    view.button('Installation finished').props.onClick(); view.navigation().onContinue();
    assert.deepEqual(item.navigations, []); assert.deepEqual(item.completed, []);
    view.pinToggle().props.onCheckedChange(false);
    assert.ok(item.render().main(), 'explicitly turning off pinning can restore a default-ref command');
  }
});
...
test('valid pinned ref and customized username survive profile changes', () => {
  ...
  item.render().picker().props.onChange({ target: { value: 'minimal' } });
  const view = item.render();
...
test('a resolver refusal never reaches executable builders or completion handlers', () => {
  const item = fixture(); item.failPlan(); item.setAcknowledged(true);
  const view = item.render();
  assert.equal(view.main(), undefined); assert.equal(view.cache(), undefined);
  assert.equal(item.calls.command.length, 0); assert.match(view.errors(), /cannot be resolved/);
  view.navigation().onContinue(); assert.deepEqual(item.completed, []);
});

test('what-installs summary is the resolved profile, not the hard-coded full tool catalogue', () => {
  const item = fixture(); const view = item.render();
  const section = view.nodes.find((node) => node.type === 'details-section' && node.props.summary === 'What this command installs');
  assert.ok(section);
  assert.match(text(section), /cloud.wrangler/); assert.match(text(section), /base.system/);
...
test('continuation still requires the installer acknowledgement', () => {
  ...
  assert.equal(view.navigation().disabled, true);
  view.navigation().onContinue(); assert.deepEqual(item.completed, []);
  item.setAcknowledged(true); view = item.render();
  assert.equal(view.navigation().disabled, false);
  view.navigation().onContinue(); assert.deepEqual(item.completed, [9]);
  assert.deepEqual(item.navigations, ['/wizard/reconnect-ubuntu']);
});

for (const [key, value] of [['profile', 'minimal'], ['mode', 'vibe'], ['ref', 'another-tag'],
  ['username', 'another-user'], ['ip', '203.0.113.43']]) {
  test(`acknowledgement cannot survive a changed ${key}`, () => {
    const item = fixture(); let view = item.render(); const previousKey = view.main().props.persistKey;
    item.setAcknowledged(true); assert.equal(item.render().navigation().disabled, false);
    item.prefs[key] = value; view = item.render();
...
test('old unbound acknowledgements do not approve a new scoped command', () => {
  const item = fixture(); item.acknowledgements.set('acfs-command-run-flywheel-installer', true);
  const view = item.render();
  assert.equal(view.navigation().disabled, true);
...
test('catalogue and checksum changes invalidate a formerly acknowledged command', () => {
  for (const key of ['manifestSha256', 'checksumsYamlSha256']) {
    const item = fixture(); let view = item.render(); const previousKey = view.main().props.persistKey;
    item.setAcknowledged(true); assert.equal(item.render().navigation().disabled, false);
    item.provenance[key] = 'c'.repeat(64); view = item.render();
    assert.notEqual(view.main().props.persistKey, previousKey);
    assert.equal(view.navigation().disabled, true);
  }
});
...
test('pending or failed hashing cannot reuse a preceding completion but leaves the safe command copyable', () => {
  const item = fixture(); item.render(); item.setAcknowledged(true);
  for (const set of [item.setCheckpointPending, item.setCheckpointFailed]) {
    set(true); const view = item.render();
...
test('late or placeholder checkpoint data from a different host cannot unlock the new context', () => {
  const item = fixture(); item.render(); item.setAcknowledged(true);
  const old = item.checkpoint(); item.prefs.ip = '203.0.113.99'; item.overrideCheckpoint(old);
  const view = item.render();
  assert.equal(view.main().props.showCheckbox, false);
  assert.equal(view.navigation().disabled, true);
});
...
test('an acknowledged context can be revisited without approving a different one', () => {
  const item = fixture(); const first = item.render().main().props.persistKey; item.setAcknowledged(true);
  item.prefs.profile = 'minimal'; assert.equal(item.render().navigation().disabled, true);
  item.prefs.profile = 'cloud-only'; const view = item.render();
...
test('cache destination follows structured command details, not a duplicate hard-coded version', () => {
  const item = fixture({ installDestination: '24.04' }); const view = item.render();
  const cacheBuilder = view.nodes.find((node) => node.type === 'command-card' && node.props.description === 'Build the verified installer cache');
