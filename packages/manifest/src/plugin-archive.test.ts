...
import { mkdtempSync, writeFileSync, symlinkSync, linkSync, truncateSync, mkdirSync, readdirSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { test } from 'node:test';
import { gzipSync } from 'node:zlib';
import {
  parsePluginJsonBytes, PluginArchiveError, PLUGIN_ARCHIVE_LIMITS,
  readPluginInputFile, readVerifiedPluginArchive,
...
interface Entry { path: string; data?: string | Buffer; kind?: string; mode?: number; prefix?: string }
...
function refuses(raw: Buffer, pattern?: RegExp): void {
  assert.throws(() => load(raw), (error: unknown) => {
...
test('requires an independent digest before reading any input', () => {
  for (const value of ['', 'a'.repeat(63), 'g'.repeat(64), ' a'.repeat(32)]) {
    assert.throws(() => readVerifiedPluginArchive('/missing/private/path', value),
      (error: unknown) => error instanceof PluginArchiveError && error.code === 'plugin_package_hash_mismatch');
  }
});

test('compares the digest before attempting decompression', () => {
  assert.throws(() => readVerifiedPluginArchive(store(Buffer.from('not gzip')), 'f'.repeat(64)),
    (error: unknown) => error instanceof PluginArchiveError && error.code === 'plugin_package_hash_mismatch');
});
...
for (const path of ['/tmp/escape', '../escape', ROOT + '../escape', ROOT + 'docs/../../escape',
  ROOT + './plugin.json', ROOT + 'docs//file', ROOT + 'docs\\file', ROOT + 'C:drive',
  ROOT + 'docs/\nfile', 'other/plugin.json', ROOT + '.hidden', ROOT + 'plugin.json/']) {
  test(`rejects unsafe member path ${JSON.stringify(path)}`, () => refuses(tar([...core(), { path }])));
}
for (const kind of ['1', '2', '3', '4', '6', '7', 'x', 'g', 'L', 'K', 'S']) {
  test(`rejects tar type ${kind}`, () => refuses(tar([...core(), { path: ROOT + 'docs/file', kind }])));
}
...
test('verifies declared static-file content and permits declared parent directories', () => {
  ...
  const manifest = JSON.stringify({ extensions: { archiveFiles: [{ path: 'docs/guide/start.md', sha256: digest(content) }] } });
  const entries = [...core(manifest), { path: ROOT + 'docs/guide/', kind: '5' },
...
for (const declaration of [null, {}, 'docs/x', [{ path: '../x', sha256: 'a'.repeat(64) }],
  [{ path: 'README.md', sha256: 'a'.repeat(64) }], [{ path: 'docs/x', sha256: 'bad' }],
...
test('rejects directory/file collisions regardless of member order', () => {
  ...
  const manifest = JSON.stringify({ extensions: { archiveFiles: [
    { path: 'docs/x', sha256: empty }, { path: 'docs/x/y', sha256: empty },
  ] } });
...
test('enforces JSON depth, node, byte and UTF-8 budgets', () => {
  assert.throws(() => parsePluginJsonBytes(Buffer.from('['.repeat(66) + '0' + ']'.repeat(66))), /budget/);
  assert.throws(() => parsePluginJsonBytes(Buffer.from(JSON.stringify(Array(50_001).fill(0)))), /budget/);
  assert.throws(() => parsePluginJsonBytes(Buffer.alloc(PLUGIN_ARCHIVE_LIMITS.manifestBytes + 1)), /budget/);
...
test('refuses symlinks, hardlinks, directories, FIFOs, empty and oversized input', () => {
  ...
  assert.throws(() => readPluginInputFile(oversized, PLUGIN_ARCHIVE_LIMITS.compressedBytes), /bounded/);
});
...
test('redacts archive and JSON diagnostics instead of echoing input', () => {
  ...
  try { readPluginInputFile(privatePath, 1024); assert.fail('accepted missing file'); }
  catch (error) {
    assert.ok(error instanceof PluginArchiveError);
    assert.ok(!error.message.includes('PRIVATE'));
    assert.ok(!error.message.includes(privatePath));
  }
  assert.throws(() => parsePluginJsonBytes(Buffer.from('{"private":"TOKEN", bad}')),
