...
import {
  ...
  openSync, opendirSync, readSync, writeFileSync, type Stats,
} from 'node:fs';
import { basename, dirname, join, parse, resolve, sep } from 'node:path';
import { gzipSync } from 'node:zlib';
import {
  PLUGIN_ARCHIVE_LIMITS as LIMITS, PluginArchiveError, verifyPluginArchiveBytes,
...
export class PluginPackError extends Error {
  constructor(public readonly code: 'plugin_pack_source_invalid' | 'plugin_pack_output_invalid', message: string) {
    super(message); this.name = 'PluginPackError';
  }
}
...
interface SourceEntry { path: string; stat: Stats; bytes: Buffer }
function refuse(message: string): never { throw new PluginPackError('plugin_pack_source_invalid', message); }
function outputError(): never {
  throw new PluginPackError('plugin_pack_output_invalid', 'Package output must be a new file outside the source, in an existing safe directory; existing or partial files are preserved');
}
...
function portable(path: string): boolean {
  return path.length <= 255 && path.split('/').every((part) => /^[A-Za-z0-9_][A-Za-z0-9._-]*$/.test(part));
}
function checkAncestors(path: string): void {
  ...
  for (const part of path.slice(current.length).split(sep).filter(Boolean)) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) refuse('Source or output has a non-directory or symlinked path component');
  }
}
function acceptable(stat: Stats): void {
  if ((!stat.isDirectory() && !stat.isFile()) || stat.isSymbolicLink()
      || (stat.isFile() && stat.nlink !== 1) || (stat.mode & 0o7022) !== 0) {
    refuse('Package sources must be ordinary directories and single-link files without special or unsafe write permissions');
  }
}
function snapshot(path: string, before: Stats, limit: number): Buffer {
  if (!Number.isSafeInteger(before.size) || before.size < 0 || before.size > limit) refuse('Package source exceeds its member size budget');
  const fd = openSync(path, constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK);
  try {
    const opened = fstatSync(fd); acceptable(opened);
    ...
    if (offset !== before.size || !same(before, fstatSync(fd))) refuse('Package source changed while reading');
...
function inventory(source: string): SourceEntry[] {
  ...
  const visit = (relative: string): void => {
    ...
    const stat = lstatSync(path); acceptable(stat);
    ...
    if (expanded > LIMITS.expandedBytes) refuse('Package source exceeds its total expanded size budget');
    const bytes = directory ? Buffer.alloc(0) : snapshot(path, stat,
...
function tarHeader(entry: SourceEntry): Buffer {
  ...
  if (name.length > 100) {
    ...
    if (split < 1 || name.length - split - 1 > 100) refuse('Package path cannot be represented by portable ustar headers');
  ...
  octal(0, 108, 8); octal(0, 116, 8); // uid/gid are never builder identities
  octal(entry.bytes.length, 124, 12); octal(0, 136, 12); // deterministic mtime
...
export function buildPluginArchive(sourceDirectory: string): PluginArchiveBuild {
  try {
    ...
    const tar = Buffer.concat([...entries.flatMap((entry) => [tarHeader(entry), entry.bytes,
      Buffer.alloc((512 - entry.bytes.length % 512) % 512)]), Buffer.alloc(1024)]);
    const bytes = gzipSync(tar, { level: 9 });
    bytes.fill(0, 4, 8); bytes[9] = 255; // no build time or builder OS metadata
...
export function writePluginArchive(build: PluginArchiveBuild, outputPath: string): PluginArchivePublication {
  const retained = copies.get(build);
  if (!retained) return refuse('Package publication requires a build produced in this process');
  try {
    const output = resolve(outputPath);
    if (output === retained.source || output.startsWith(retained.source + sep)
        || !/^[A-Za-z0-9_][A-Za-z0-9._-]*\.tar\.gz$/.test(basename(output))) return outputError();
    const parent = dirname(output); checkAncestors(parent);
    ...
    try {
      ...
      writeFileSync(fd, retained.bytes); fchmodSync(fd, 0o600); fsyncSync(fd);
      const current = lstatSync(output);
      if (current.dev !== stat.dev || current.ino !== stat.ino || current.nlink !== 1) return outputError();
    } finally { closeSync(fd); }
    ...
    return { packageSha256: build.packageSha256, compressedBytes: build.compressedBytes,
      expandedBytes: build.expandedBytes, fileCount: build.fileCount, reviewRequired: true };
  } catch (error) {
