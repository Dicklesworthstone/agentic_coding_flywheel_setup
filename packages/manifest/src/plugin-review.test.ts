...
import { parsePluginTarget, readPluginReviewRecord, readReviewedPluginArchive, PluginReviewError } from './plugin-review.js';
...
const base = () => ({
  schema: 'acfs.plugin-review.v1', packageId: 'example.tools', version: '1.2.3',
  sourceCommit: 'a'.repeat(40), packageSha256: 'b'.repeat(64), reviewer: 'maintainer',
  reviewedAt: '2026-09-16T00:00:00Z', expiresAt: '2026-09-18T00:00:00Z', target,
...
function fixture(): { archive: string; review: ReturnType<typeof base>; manifest: Record<string, unknown> } {
  const manifest = { packageId: 'example.tools', version: '1.2.3', provenance: { sourceCommit: 'a'.repeat(40) },
  ...
  for (const [name, data] of [['plugin.json', JSON.stringify(manifest)], ['README.md', 'Example'], ['LICENSE', 'MIT']]) {
    writeFileSync(join(staging, 'acfs-plugin-package', name!), data!);
  }
  const bytes = execFileSync('tar', ['--format=ustar', '-czf', '-', '-C', staging, 'acfs-plugin-package']);
...
test('binds one external review to compressed bytes, package identity and explicit target', () => {
  const item = fixture();
  assert.deepEqual(readReviewedPluginArchive(item.archive, jsonFile(item.review), target, NOW).manifest, item.manifest);
});
...
for (const invalid of ['', 'ubuntu/26.04/x86_64', 'ubuntu/26.04/x86_64/glibc/extra',
  'ubuntu//x86_64/glibc', 'ubuntu/26.04/../../glibc', 'ubuntu/26.04/x86_64/glibc\n']) {
...
for (const mutation of [
  { expiresAt: '2026-09-17T12:00:00Z' }, { expiresAt: '2026-09-16T00:00:00Z' },
  { reviewedAt: '2026-09-18T00:00:00Z' }, { reviewedAt: '2026-02-30T00:00:00Z' },
  { expiresAt: 'September 18, 2026' }, { expiresAt: '2026-09-18T00:00:00+00:00' },
  { approvedCapabilities: ['root_run_as'] }, { approvedCapabilities: ['default_enabled_module'] },
  { approvedCapabilities: ['verified_installer', 'verified_installer'] },
  { approvedCapabilities: [] }, { approvedCapabilities: [7] }, { approvedCapabilities: 'doctor_check' },
  { schema: 'acfs.plugin-review.v2' }, { ignored: true }, { packageSha256: 'invalid' },
  { sourceCommit: 'short' }, { reviewer: '   ' }, { version: '1.2.3\n' }, { target: { ...target, ignored: true } },
]) {
  test(`fails closed on review mutation ${JSON.stringify(mutation)}`, () => {
    assert.throws(() => readPluginReviewRecord(jsonFile({ ...base(), ...mutation }), NOW), PluginReviewError);
  });
}
...
test('refuses identity substitutions and incomplete capability approval', () => {
  const item = fixture();
  for (const mutation of [{ packageId: 'other.tools' }, { version: '1.2.4' },
    { sourceCommit: 'c'.repeat(40) }, { approvedCapabilities: ['doctor_check'] }]) {
    assert.throws(() => readReviewedPluginArchive(item.archive, jsonFile({ ...item.review, ...mutation }), target, NOW), PluginReviewError);
  }
});

test('refuses review target mismatch before opening the archive', () => {
  assert.throws(() => readReviewedPluginArchive('/missing/SECRET', jsonFile(base()),
    parsePluginTarget('ubuntu/26.04/aarch64/glibc'), NOW), /differs from the external review/);
});
...
test('refuses targets that are reviewed but unsupported by the archived manifest', () => {
  ...
  assert.throws(() => readReviewedPluginArchive(item.archive, jsonFile({ ...item.review, target: other }), other, NOW), /complete requested target/);
});

test('does not accept review-like metadata bundled inside the archive instead of a local record', () => {
  const item = fixture();
  assert.throws(() => readReviewedPluginArchive(item.archive, item.archive, target, NOW), /External review record/);
});
...
test('rejects duplicate review keys with redacted errors', () => {
  ...
  try { readPluginReviewRecord(path, NOW); assert.fail('accepted duplicate keys'); }
  catch (error) {
