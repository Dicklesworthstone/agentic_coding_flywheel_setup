...
export type PluginInstallerDownload = (
...
function refuse(code: string, message: string): never {
  throw new PluginCacheError(code, message);
}
...
function clock(now: number): number {
  if (
    ...
    now < 0 ||
    now > 8_640_000_000_000_000 - PLUGIN_CACHE_LIMITS.lifetimeMs
  ) {
...
function canonicalTime(value: unknown): number {
  if (typeof value !== "string")
    return refuse("plugin_cache_invalid", "Plugin cache timestamp is invalid");
...
function snapshotPlan(plan: PluginInstallPlan): PluginInstallPlan {
  ...
  if (
    ...
    copy.schema !== "acfs.plugin-install-plan.v1" ||
    !HEX.test(copy.package.pluginSha256) ||
    !Array.isArray(copy.actions) ||
    copy.actions.length < 1 ||
    copy.actions.length > PLUGIN_CACHE_LIMITS.entries ||
    new Set(copy.actions.map((action) => action.id)).size !== copy.actions.length
  ) {
    return refuse(
      "plugin_cache_plan_mismatch",
      "Plugin cache requires an unchanged reviewed install plan",
    );
  }
  for (const action of copy.actions) {
    const installer = action.installer;
    if (
      !/^plugin\.[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)+$/.test(action.id) ||
      !/^[a-z][a-z0-9_]*$/.test(installer.tool) ||
      !HEX.test(installer.sha256) ||
...
function realDirectory(path: string, allowStickyParent = false): void {
  ...
  for (const part of path.slice(current.length).split("/").filter(Boolean)) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
      refuse(
        "plugin_cache_path_unsafe",
        "Plugin cache has a non-directory or symlinked path component",
      );
    }
  }
  ...
  if (
    (stat.mode & 0o022) !== 0 &&
    !(allowStickyParent && (stat.mode & 0o1000) !== 0 && stat.uid === 0)
  ) {
...
function readSnapshot(path: string, limit: number): Buffer {
  ...
  try {
    ...
    if (
      ...
      refuse(
        "plugin_cache_file_unsafe",
        "Cache members must be bounded, nonempty single-link regular files without unsafe permissions",
      );
    }
...
function summary(manifest: CacheManifest, blobs: Map<string, Buffer>): PluginCacheSummary {
  return {
    ...
    artifactCount: blobs.size,
    totalBytes: [...blobs.values()].reduce((total, bytes) => total + bytes.length, 0),
...
function safeError(error: unknown): never {
  ...
  return refuse(
    "plugin_cache_unavailable",
    "Plugin cache could not be read or prepared safely; no live entrypoint fallback is permitted",
  );
}
  ...
  try {
    ...
    for (const action of plan.actions) {
      ...
      if (!bytes) {
        ...
        if (
          !bytes.length ||
          bytes.length > PLUGIN_CACHE_LIMITS.entrypointBytes ||
          hash(bytes) !== action.installer.sha256
        ) {
          refuse(
            "plugin_cache_hash_mismatch",
            "Entrypoint bytes do not match the reviewed canonical digest",
          );
        }
      ...
      if (!blobs.has(path)) {
        total += bytes.length;
        if (total > PLUGIN_CACHE_LIMITS.totalBytes)
          refuse("plugin_cache_too_large", "Selected entrypoints exceed the cache size budget");
      ...
      entries.push({
        moduleId: action.id,
        tool: action.installer.tool,
        url: action.installer.url,
        sha256: action.installer.sha256,
    ...
    const manifest: CacheManifest = {
      ...
      expiresAt: new Date(generatedAt + PLUGIN_CACHE_LIMITS.lifetimeMs).toISOString(),
      policy: POLICY,
      entries,
    };
    const manifestBytes = Buffer.from(serialize(manifest));
    if (manifestBytes.length > PLUGIN_CACHE_LIMITS.manifestBytes)
      refuse("plugin_cache_too_large", "Cache metadata exceeds its size budget");
    cancelled(signal);
    realDirectory(dirname(output), true);
    ...
    for (const [path, bytes] of blobs) {
      cancelled(signal);
      exclusiveWrite(join(output, path), bytes);
    }
...
export function loadPluginInstallerCache(
  ...
  try {
    ...
    realDirectory(root);
    realDirectory(join(root, "scripts"));
    ...
    if (
      ...
      !record(data.policy) ||
      !keys(data.policy, "entrypointFetchMode,executionNetworkMode,transitiveClosure") ||
      ...
      refuse(
        ...
        "Plugin cache schema, canonical JSON, or entrypoint-only policy is invalid",
      );
    }
    if (data.planSha256 !== plan.planSha256 || data.packageSha256 !== plan.package.pluginSha256) {
      refuse(
        "plugin_cache_plan_mismatch",
        "Cache does not match the current reviewed selection, target, package, or trust inputs",
      );
    }
    ...
    if (
      generated > now ||
      expires <= now ||
      expires - generated !== PLUGIN_CACHE_LIMITS.lifetimeMs
    ) {
      refuse(
        "plugin_cache_expired",
        "Plugin cache is expired, future-dated, or has an invalid validity interval",
      );
    }
    if (!Array.isArray(data.entries) || data.entries.length !== plan.actions.length) {
      refuse(
        "plugin_cache_incomplete",
        "Cache must cover every selected plugin action exactly once",
      );
    }
    const blobs = new Map<string, Buffer>();
    ...
    for (const entry of data.entries) {
      ...
      if (
        !record(entry) ||
        !keys(entry, "moduleId,path,sha256,sizeBytes,tool,url") ||
      ...
      if (
        !action ||
        byAction.has(entry.moduleId) ||
        entry.tool !== action.installer.tool ||
        entry.url !== action.installer.url ||
        entry.sha256 !== action.installer.sha256 ||
        ...
        refuse(
          "plugin_cache_plan_mismatch",
          "Cache entry is missing, duplicated, unsafe, or differs from the reviewed installer",
        );
      }
      ...
      if (!script) {
        ...
        total += script.length;
        if (total > PLUGIN_CACHE_LIMITS.totalBytes)
          refuse("plugin_cache_too_large", "Cache exceeds its total entrypoint budget");
        if (hash(script) !== action.installer.sha256)
          refuse(
            "plugin_cache_hash_mismatch",
            "Cached entrypoint digest does not match canonical checksums",
          );
        blobs.set(path, script);
      }
      if (script.length !== entry.sizeBytes)
        refuse("plugin_cache_hash_mismatch", "Cached entrypoint size differs from its declaration");
    ...
    if (
      JSON.stringify(readdirSync(join(root, "scripts")).sort()) !== JSON.stringify(expectedFiles)
    ) {
    ...
    return {
      ...
      download: async (action, signal) => {
        ...
        if (!expected || expected.action !== JSON.stringify(action)) {
          return refuse(
            "plugin_cache_plan_mismatch",
            "Requested action was not verified in this cache snapshot",
          );
        }
