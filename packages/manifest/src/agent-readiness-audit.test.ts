...
class FixtureFileSystem implements AgentReadinessFileSystem {
  ...
  constructor(entries: Record<string, FixtureEntry>) {
    this.entries = new Map(Object.entries(entries).map(([path, entry]) => [resolve(path), entry]));
  }
...
function baseEntries(): Record<string, FixtureEntry> {
  return {
    ...
    [`${HOME}/.claude/.credentials.json`]: jsonFile({
      claudeAiOauth: { accessToken: REDACTION_SAMPLE },
    }),
...
function createCliFixture() {
  ...
  writeRealFile(
    join(home, ".config", "caam", "config.json"),
    JSON.stringify({
      default_profiles: {
        claude: "work",
        codex: "work",
...
function runAuditCli(
  ...
  return spawnSync(
    process.execPath,
    [AUDIT_SCRIPT, ...args, "--home", fixture.home, "--path", fixture.bin],
    {
      encoding: "utf8",
      env: {
        HOME: fixture.home,
