...
const options = () => ({
  target,
  firstPartyManifest: {
    version: 1, name: 'Plugin verification fixture', id: 'acfs',
    defaults: { user: 'ubuntu', workspace_root: '/data/projects', mode: 'vibe' },
    modules: [{ id: 'base.system', description: 'Base system', category: 'base', phase: 1,
      run_as: 'target_user', optional: false, enabled_by_default: true, generated: true,
      install: ['echo base'], verify: ['true'] }],
  } as Manifest,
...
function plugin() {
  return {
    ...
    displayName: 'Example Tools', version: '1.2.3', description: 'Example plugin.',
    publisher: { name: 'Example Maintainers', contactUrl: 'https://example.com/security',
      sourceUrl: 'https://github.com/example/tools' },
    license: 'MIT', docsUrl: 'https://example.com/docs',
    provenance: { generatedAt: '2026-05-08T00:00:00Z', sourceRef: 'main',
      sourceCommit: 'a'.repeat(40), acfsManifestVersion: 1 },
    targets: [{ os: target.os, versions: [target.version], arch: [target.arch], libc: [target.libc] }],
    ...
    modules: [{ id: 'plugin.example_tools.cli', description: 'Example command-line tool.',
      category: 'tools', phase: 6, run_as: 'target_user', optional: false, enabled_by_default: false,
      dependencies: ['base.system'], install: { kind: 'verified_installer', tool: 'example_tools',
        url: 'https://example.com/install.sh', runner: 'bash', args: [], env: [] },
      verify: [{ kind: 'command_exists', command: 'example' }], docs_url: 'https://example.com/docs' }],
    offline: { bundlingPolicy: 'metadata_only', liveAuthRequired: false, providerInteractionRequired: false },
    extensions: {},
  };
}
function fixture(value = plugin()) {
  ...
  const bytes = execFileSync('tar', ['--format=ustar', '-czf', '-', '-C', directory, 'acfs-plugin-package']);
  ...
  writeFileSync(review, JSON.stringify({
    schema: 'acfs.plugin-review.v1', packageId: value.packageId, version: value.version,
    sourceCommit: value.provenance.sourceCommit, packageSha256, reviewer: 'Test maintainer',
    reviewedAt: new Date(Date.now() - 60_000).toISOString(),
    expiresAt: new Date(Date.now() + 60_000).toISOString(), target,
...
test('CLI arguments require explicit archive, review and target', () => {
  assert.deepEqual(parsePluginVerifyArguments(['--archive', 'a.tar.gz', '--review', 'r.json',
    '--target', 'ubuntu/26.04/x86_64/glibc', '--json']), { archive: 'a.tar.gz', review: 'r.json', target, json: true });
  for (const args of [[], ['--archive'], ['--archive', '--json'], ['--unknown', 'SECRET'],
    ['--json', '--json'], ['--target', 'ubuntu/26.04/x86_64/glibc', '--target', 'another']]) {
...
test('CLI help and invalid-input JSON are runnable without loading package validators', () => {
  ...
  const failure = spawnSync(process.execPath, [script, '--archive', 'PRIVATE_PATH', '--json'], { encoding: 'utf8' });
...
test('canonical integration requires the first-party installer checksum and exact URL', async () => {
  const item = fixture();
  for (const installers of [{}, { example_tools: { url: 'https://example.com/other.sh', sha256: 'b'.repeat(64) } }]) {
    const result = await loadReviewedPluginPackage(item.archive, item.review, { ...options(), installers });
    assert.equal(result.valid, false);
    assert.equal(result.manifestModules.length, 0);
    assert.ok(result.diagnostics.some((entry) => entry.code === 'plugin_verified_installer_checksum_required'));
  }
});
