...
export interface PluginPlanTarget { os: string; version: string; arch: string; libc: string }
export interface PluginPlanProvenance {
  packageId: string; version: string; sourceCommit: string; pluginSha256: string;
}
...
export interface PlannablePluginModule {
  ...
  verified_installer?: {
    tool: string; url?: string; runner: string; args?: string[]; env?: string[]; fallback_url?: string;
  };
}
export interface PluginPlanPrerequisite {
  id: string; phase?: number; dependencies?: string[]; verify: string[];
}
...
export class PluginPlanError extends Error {
  ...
  constructor(message: string) { super(message); this.name = 'PluginPlanError'; }
}
const ID = /^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)*$/;
const HEX = /^[a-f0-9]{64}$/i;
const LIMIT = 1024;
function refuse(message: string): never { throw new PluginPlanError(message); }
function compare(a: string, b: string): number { return a < b ? -1 : a > b ? 1 : 0; }
function selections(value: readonly string[], label: string): string[] {
  if (!Array.isArray(value) || value.length > LIMIT || value.some((id) => typeof id !== 'string' || !ID.test(id))) {
...
export function buildPluginInstallPlan(input: PluginPlanInput): PluginInstallPlan {
  ...
  if (Object.keys(input.target).sort().join(',') !== 'arch,libc,os,version'
      || Object.values(input.target).some((part) => typeof part !== 'string' || !/^[a-z0-9][a-z0-9_.-]{0,63}$/.test(part))) {
  ...
  if (plugins.size !== input.modules.length || firstParty.size !== input.firstPartyModules.length
      || [...plugins.keys()].some((id) => firstParty.has(id))) refuse('Duplicate module IDs in the merged graph');
  for (const module of [...input.firstPartyModules, ...input.modules]) {
    if (!ID.test(module.id) || !Number.isInteger(module.phase ?? 1)
        || (module.phase ?? 1) < 1 || (module.phase ?? 1) > 10) refuse('Invalid module identity or phase');
  ...
  const visit = (id: string): void => {
    ...
    for (const dependency of [...(module.dependencies ?? [])].sort(order)) {
      if (firstParty.has(id) && plugins.has(dependency)) refuse('First-party prerequisites cannot depend on plugin modules');
      if (phase(dependency) > phase(id)) refuse('A dependency executes in a later phase than its dependent');
      visit(dependency);
    }
    visiting.delete(id);
    visited.add(id);
  ...
  if (!provenance || !/^[a-z][a-z0-9_.-]{0,127}$/.test(provenance.packageId)
      || !HEX.test(provenance.pluginSha256) || !/^[a-f0-9]{40}$/i.test(provenance.sourceCommit)
      || !provenance.version || /[\x00-\x1f\x7f]/.test(provenance.version)) {
  ...
  for (const id of ordered) {
    ...
    if (!module) {
      ...
      if (!Array.isArray(dependency.verify) || !dependency.verify.length
          || dependency.verify.some((check) => typeof check !== 'string' || !check.trim() || check.includes('\0'))) {
        refuse('First-party prerequisite has no usable verification checks');
      }
      prerequisites.push({ id, verify: [...dependency.verify] });
      continue;
    }
    if (!id.startsWith(`plugin.${slug}.`) || !module.plugin
        || module.plugin.packageId !== provenance.packageId || module.plugin.version !== provenance.version
    ...
    if (module.run_as !== 'target_user' || module.enabled_by_default || module.install.length !== 0) {
      refuse('Only explicitly selected target-user verified installers are executable');
    }
    const installer = module.verified_installer;
    const trusted = installer && Object.hasOwn(input.installers, installer.tool) ? input.installers[installer.tool] : undefined;
    if (!installer || !/^[a-z][a-z0-9_]*$/.test(installer.tool) || !trusted
        || !HEX.test(trusted.sha256) || trusted.url !== installer.url
        || !['bash', 'sh'].includes(installer.runner) || (installer.env?.length ?? 0) !== 0
        || installer.fallback_url !== undefined) refuse('Installer is not bound to canonical checksums and an allowed runner');
    let url: URL;
    try { url = new URL(trusted.url); } catch { return refuse('Installer URL is invalid'); }
    if (url.protocol !== 'https:' || url.username || url.password || url.hash) refuse('Installer URL must be credential-free HTTPS');
    const args = installer.args ?? [];
    if (!Array.isArray(args) || args.length > 128 || args.some((arg) => typeof arg !== 'string'
        || arg === '--' || arg.length > 4096 || /[\x00-\x1f\x7f]/.test(arg))) refuse('Installer arguments are invalid');
    if (!Array.isArray(module.verify) || !module.verify.length) refuse('Plugin module has no verification checks');
    const verify = module.verify.map((check) => {
      const match = /^command -v -- ([A-Za-z0-9][A-Za-z0-9._+-]*) >\/dev\/null 2>&1$/.exec(check);
      if (!match) return refuse('Plugin verification must be a canonical executable-existence check');
      return match[1]!;
    });
    actions.push({ id, installer: { tool: installer.tool, url: trusted.url, sha256: trusted.sha256.toLowerCase(),
      runner: installer.runner as 'bash' | 'sh', args: [...args] }, verify });
  }
  const payload = {
    schema: 'acfs.plugin-install-plan.v1' as const,
    package: { packageId: provenance.packageId, version: provenance.version,
      sourceCommit: provenance.sourceCommit.toLowerCase(), pluginSha256: provenance.pluginSha256.toLowerCase() },
    target: { os: input.target.os, version: input.target.version, arch: input.target.arch, libc: input.target.libc },
    trust: { manifestSha256: input.trust.manifestSha256.toLowerCase(), checksumsSha256: input.trust.checksumsSha256.toLowerCase() },
    requested, skipped, dependencyIds: ordered.filter((id) => !requested.includes(id)), prerequisites, actions,
  };
