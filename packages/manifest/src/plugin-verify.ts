...
import { parsePluginTarget, PluginReviewError, readReviewedPluginArchive } from './plugin-review.js';
import type { PluginValidationOptions, PluginValidationResult, PluginValidationTarget } from './plugin.js';
...
export async function loadReviewedPluginPackage(
  ...
  try {
    ...
    if (!ManifestSchema.safeParse(merged).success || !validateManifestData(merged).valid
        || !validateManifest(merged).valid
        || validateVerifiedInstallerChecksums(merged, options.installers ?? {}).length > 0) {
      return { valid: false, manifestModules: [], diagnostics: [{
        code: 'plugin_dependency_invalid', severity: 'error', path: '<merged-manifest>',
        message: 'Merged plugin graph failed canonical schema, dependency, phase, or checksum validation',
      }] };
    }
    return result;
  } catch (error) {
    if (error instanceof PluginArchiveError || error instanceof PluginReviewError) {
      return { valid: false, manifestModules: [], diagnostics: [{
        code: error.code, severity: 'error', path: '<package>', message: error.message,
      }] };
    }
    // Do not echo filesystem paths, loader errors, or untrusted package content.
    return { valid: false, manifestModules: [], diagnostics: [{
      code: 'plugin_disallowed_behavior', severity: 'error', path: '<validation>',
      message: 'Plugin verification could not complete; check the local manifest tooling and trust inputs',
    }] };
  }
}

export interface PluginVerifyArguments { archive: string; review: string; target: PluginValidationTarget; json: boolean }
export function parsePluginVerifyArguments(args: readonly string[]): PluginVerifyArguments {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (!value || value.startsWith('--')) throw new Error('Plugin verification option requires a value');
    values.set(option, value);
  }
  if (values.size !== 3) throw new Error('Explicit --archive, --review and --target values are required');
  return { archive: values.get('--archive')!, review: values.get('--review')!,
    target: parsePluginTarget(values.get('--target')!), json };
}

export async function pluginVerifyMain(args: readonly string[]): Promise<number> {
  if (args.length === 1 && (args[0] === '--help' || args[0] === '-h')) {
    console.log('Usage: bun run plugin:verify --archive package.tar.gz --review trusted-review.json --target os/version/arch/libc [--json]\nRead-only verification. Never installs plugins or changes generated output.');
    return 0;
  }
  const json = args.includes('--json');
  let output: { valid: boolean; activation: 'disabled'; diagnostics: unknown[]; moduleCount?: number };
  let exitCode: number;
  try {
    ...
    const manifest = parseManifestString(readPluginInputFile(join(root, 'acfs.manifest.yaml'), 4 * 1024 * 1024).toString('utf8'));
    const checksums = parseYaml(readPluginInputFile(join(root, 'checksums.yaml'), 1024 * 1024).toString('utf8')) as {
      installers?: Record<string, InstallerChecksumEntry>;
    };
    if (!manifest.success || !manifest.data || !checksums || typeof checksums.installers !== 'object'
        || checksums.installers === null || Array.isArray(checksums.installers)) {
    ...
    const result = await loadReviewedPluginPackage(options.archive, options.review, {
      firstPartyManifest: manifest.data, installers: checksums.installers, target: options.target,
    });
    output = { valid: result.valid, activation: 'disabled', diagnostics: result.diagnostics,
    ...
    output = { valid: false, activation: 'disabled', diagnostics: [{
      code: known ? error.code : 'plugin_verification_input_invalid',
      severity: 'error', path: '<input>',
      message: known ? error.message : 'Verification arguments or canonical trust inputs are invalid or unavailable',
    }] };
    exitCode = known ? 1 : 2;
  }
