...
function module(id: string, dependencies: string[] = []): PlannablePluginModule {
  return {
    ...
    dependencies,
    install: [],
    verify: [`command -v -- ${id} >/dev/null 2>&1`],
    plugin: {
      packageId: "example",
      version: "1.0.0",
...
function input(): PluginPlanInput {
  return {
    modules: [module("app", ["plugin.example.lib", "lang.bun"]), module("lib"), module("unused")],
    firstPartyModules: [
      { id: "lang.bun", phase: 6, dependencies: ["base.system"], verify: ["bun --version"] },
...
test("explicit selection includes transitive dependencies but never the rest of the package", () => {
  ...
  assert.deepEqual(
    ...
    ["plugin.example.lib", "plugin.example.app"],
  );
  assert.deepEqual(
    plan.prerequisites.map((entry) => entry.id),
    ["base.system", "lang.bun"],
  );
...
test("same selected graph is deterministic across module and dependency enumeration", () => {
  const config = input();
  const plan = buildPluginInstallPlan(config);
  config.modules = [...config.modules].reverse();
  config.firstPartyModules = [...config.firstPartyModules].reverse();
...
for (const only of [
  ...
  ["base.system"],
  ["../escape"],
  ["plugin.example.app", "plugin.example.app"],
  [1] as unknown as string[],
]) {
  test(`refuses invalid explicit selection ${JSON.stringify(only)}`, () =>
    invalid((config) => {
      config.only = only;
    }));
}
for (const skip of [
  ...
  test(`refuses contradictory or malformed skips ${JSON.stringify(skip)}`, () =>
    invalid((config) => {
      config.skip = skip;
    }));
}
...
test("allows skipping unselected modules without widening the plan", () => {
  const config = input();
  config.skip = ["plugin.example.unused"];
...
for (const issue of ["cycle", "unknown", "phase", "duplicate", "first-party-edge"]) {
  test(`rejects invalid dependency graph: ${issue}`, () =>
    invalid((config) => {
      ...
      if (issue === "first-party-edge")
        config.firstPartyModules[1]!.dependencies = ["plugin.example.lib"];
    }));
}
for (const change of [
  (config: PluginPlanInput) => {
    config.modules[0]!.run_as = "root";
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.run_as = "current";
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.enabled_by_default = true;
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.install = ["touch injected"];
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verify = ["echo injected"];
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verify = [];
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.plugin!.pluginSha256 = "f".repeat(64);
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.plugin!.packageId = "other";
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verified_installer!.env = ["BASH_ENV=attack"];
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verified_installer!.runner = "python";
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verified_installer!.fallback_url = "https://example.com/evil";
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verified_installer!.args = ["--"];
  },
  (config: PluginPlanInput) => {
    config.modules[0]!.verified_installer!.args = ["\0"];
  },
  (config: PluginPlanInput) => {
    config.installers = {};
  },
  (config: PluginPlanInput) => {
    config.installers.example!.sha256 = "bad";
  },
  (config: PluginPlanInput) => {
    config.installers.example!.url = "https://example.com/other";
  },
  (config: PluginPlanInput) => {
    config.trust.manifestSha256 = "";
  },
  (config: PluginPlanInput) => {
    config.target.libc = "";
  },
  (config: PluginPlanInput) => {
    config.firstPartyModules[0]!.verify = [];
  },
]) {
...
test("binds every execution-relevant change into the plan fingerprint", () => {
  ...
  for (const change of [
    (config: PluginPlanInput) => {
      config.modules[0]!.verified_installer!.args = ["--yes"];
    },
    (config: PluginPlanInput) => {
      config.modules[0]!.verify = ["command -v -- other >/dev/null 2>&1"];
    },
    (config: PluginPlanInput) => {
      config.installers.example!.sha256 = "f".repeat(64);
    },
    (config: PluginPlanInput) => {
      config.firstPartyModules[0]!.verify = ["bun --revision"];
    },
    (config: PluginPlanInput) => {
      config.trust.checksumsSha256 = "f".repeat(64);
    },
    (config: PluginPlanInput) => {
      config.target.arch = "aarch64";
    },
  ]) {
    const config = input();
    change(config);
...
test("refuses inherited installer entries", () =>
  invalid((config) => {
