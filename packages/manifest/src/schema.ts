 ...
 */
export const ManifestDefaultsSchema = z.strictObject({
  user: z
    .string()
...
function isAllowedVerifiedInstallerEnv(tool: string, entry: string): boolean {
  ...
  if (tool === "mcp_agent_mail") {
    return (
      entry === "AM_INSTALL_SKIP_MCP_SETUP=1" || entry === "AM_INSTALL_SKIP_REMOTE_HTTP_READINESS=1"
    );
  }
...
const VerifiedInstallerSchema = z
  .strictObject({
    ...
    url: z
      ...
      .refine((value) => value.startsWith("https://"), "verified_installer.url must use https://")
      .optional(),
    fallback_url: z
      .string()
 ...
 */
export const ModuleWebMetadataSchema = z.strictObject({
  display_name: z
    .string()
  ...
  color: z
    .string()
    .regex(/^#[0-9a-fA-F]{6}$/, 'color must be a 6-digit hex code (e.g., "#3B82F6")')
    .optional(),
  category_label: z
    .string()
  ...
  cli_name: z
    ...
    .regex(/^[a-z][a-z0-9_-]*$/, "cli_name must be lowercase alphanumeric with hyphens/underscores")
    .optional(),
  cli_aliases: z.array(z.string().min(1).max(30)).max(10).optional(),
  command_example: z
    .string()
 ...
 */
export const ModuleAgentMetadataSchema = z.strictObject({
  display_name: z
    .string()
  ...
  docs_url: z
    ...
    .refine((value) => value.startsWith("https://"), "agent.docs_url must use https://"),
  summary: z
    .string()
...
export const ModuleSchema = z
  .strictObject({
    ...
    pre_install_check: z
      .strictObject({
        ...
        command: z
          ...
          .refine(
            (s) => s.trim().length > 0,
            "Pre-install check command cannot be only whitespace",
          ),
        skip_message: z
          .string()
          ...
          .refine(
            (s) => s.trim().length > 0,
            "Pre-install check skip_message cannot be only whitespace",
          ),
      })
      .optional(),
    ...
    post_install_message: z
      ...
      .refine((s) => s.trim().length > 0, "post_install_message cannot be only whitespace")
      .optional(),
  ...
  .refine((module) => module.verified_installer === undefined || module.run_as === "target_user", {
    path: ["run_as"],
    message:
      "Modules with verified_installer must run as target_user through the clean runner boundary.",
  })
