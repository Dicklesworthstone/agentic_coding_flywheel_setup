  ...
  while (offset + 512 <= tar.length) {
    ...
    const limit =
      relative === "plugin.json"
        ? PLUGIN_ARCHIVE_LIMITS.manifestBytes
        : PLUGIN_ARCHIVE_LIMITS.memberBytes;
...
function validateMembers(members: Map<string, Member>, manifest: unknown): number {
  const files = new Map<string, string | undefined>(
    [...CORE_FILES].map((path) => [path, undefined]),
  );
  ...
  if (declarations !== undefined) {
    ...
    for (const entry of declarations) {
      if (
        ...
        !/^(assets|docs|provenance)\/.+/.test(entry.path) ||
...
export function readVerifiedPluginArchive(
  ...
  if (!isDigest(expectedPackageSha256)) {
    throw new PluginArchiveError(
      "plugin_package_hash_mismatch",
      "An independently trusted SHA-256 is required",
    );
  }
...
function verifySnapshot(compressed: Buffer, expectedPackageSha256: string): VerifiedPluginArchive {
  ...
  if (packageSha256 !== expectedPackageSha256.toLowerCase()) {
    throw new PluginArchiveError(
      "plugin_package_hash_mismatch",
      "Plugin package digest does not match the trusted digest",
    );
  }
  let tar: Buffer;
  try {
  ...
  if (!manifestFile || manifestFile.directory)
    invalid("Plugin archive is missing its regular plugin.json");
