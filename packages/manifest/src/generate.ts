...
function shellQuoteVerifiedInstallerArg(str: string): string {
  ...
  const variablePattern =
    /(\$\{TARGET_USER:-ubuntu\}|\$\{TARGET_HOME\}|\$TARGET_HOME|\$\{TARGET_USER\}|\$TARGET_USER|\$\$)/g;
...
function encodeDoctorCommand(cmd: string): string {
  return cmd.replace(/\\/g, "\\\\").replace(/\t/g, "\\t").replace(/\r?\n/g, "\\n");
}
...
function moduleFailureLines(module: Module, reason: string): string[] {
  ...
  return [`log_error "${module.id}: ${escapedReason}"`, "return 1"];
}
...
function commandLinesWithChildHelperPreludes(commandLines: string[]): string[] {
  ...
  return [...preludeLines, ...commandLines];
}
...
function generateVerifiedInstallerSnippet(module: Module): string[] {
  ...
  const fsfsInstallerArgs =
    normalizedScriptArgs.length > 0
...
function summarizeShellBlock(blockLines: string[], fallback: string): string {
  ...
  for (const line of blockLines) {
    ...
    if (skippingFunction) {
      ...
      if (
        !skippingFunctionQuoteState.double &&
        !skippingFunctionQuoteState.single &&
        trimmed === "}"
      ) {
...
export function generateInternalChecksums(
  ...
  for (const relPath of INTERNAL_SCRIPTS_TO_CHECKSUM) {
    ...
    if (pendingGeneratedFile) {
      ...
      content =
        boundStaticSnapshots.get(absPath) ??
...
export function generateDoctorChecks(manifest: Manifest): string {
  ...
  for (const module of sortedModules) {
    ...
    for (let i = 0; i < module.verify.length; i++) {
      ...
      lines.push(
        `    "${checkId}${suffix}\t${description}\t${escapeBash(encodedCmd)}\t${isOptional ? "optional" : "required"}\t${module.run_as}"`,
      );
    }
  }
...
export function generateTopLevelInstaller(manifest: Manifest): string {
  ...
  const lines: string[] = [
    sourceOnlyHeader("ERROR: install_all.sh is a source-only generated harness; run install.sh"),
  ];
...
export function generateWebTools(manifest: Manifest): string {
  ...
  for (const module of modules) {
    ...
    lines.push(
      `    shortName: "${escapeTs(web.short_name ?? module.id.split(".").pop() ?? module.id)}",`,
    );
...
export function generateWebTldr(manifest: Manifest): string {
  ...
  for (const module of modules) {
    ...
    lines.push(`    tagline: "${escapeTs(web.tagline ?? module.description)}",`);
    lines.push(
      `    tldrSnippet: "${escapeTs(web.tldr_snippet ?? web.short_desc ?? module.description)}",`,
    );
    lines.push(`    icon: "${escapeTs(web.icon ?? "box")}",`);
    lines.push(`    color: "${escapeTs(web.color ?? "#6B7280")}",`);
    if (web.href) {
...
export const README_AGENT_SUMMARY_END = "<!-- END GENERATED: compatible-agents-summary -->";
...
export function buildAgentRoster(manifest: Manifest): AgentRosterEntry[] {
  return manifest.modules
    ...
    .sort((a, b) => {
      const byStatus = AGENT_ROSTER_STATUS_ORDER[a.status] - AGENT_ROSTER_STATUS_ORDER[b.status];
...
export function generateAgentRosterMarkdown(manifest: Manifest): string {
  ...
  for (const entry of roster) {
    ...
    const aliases = entry.aliases.length > 0 ? entry.aliases.map(markdownCode).join(", ") : "—";
    lines.push(
  ...
  for (const entry of roster) {
    lines.push(
      `- **${escapeMarkdownCell(entry.displayName)}** — ${escapeMarkdownCell(entry.summary)}`,
    );
  }
...
export function generateWebIndex(): string {
  ...
  lines.push(
    "export type { ManifestModuleMetadata, ManifestSelectionProfile, ManifestSelectionProfileId, ManifestProvenanceMetadata, ManifestPluginProvenance } from './manifest-modules';",
  );
  lines.push("");
...
export function collectPluginInputPaths(
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    if (argument === "--plugin" || argument === "--plugins") {
      ...
      if (!value.trim()) {
        throw new Error(
          `${argument.slice(0, argument.indexOf("="))} requires a plugin package path`,
        );
      }
      paths.push(resolve(cwd, value));
    } else if (argument === "--plugins-dir" || argument === "--plugin-dir") {
      appendPluginDirectory(paths, pluginOptionValue(args, index, argument), argument, cwd);
      index++;
    } else if (argument.startsWith("--plugins-dir=") || argument.startsWith("--plugin-dir=")) {
      const option = argument.slice(0, argument.indexOf("="));
      appendPluginDirectory(paths, argument.slice(argument.indexOf("=") + 1), option, cwd);
...
export function enforceRawPluginActivationBoundary(
  ...
  const hasPluginOption = args.some(
    (argument) =>
      argument === "--plugin" ||
      argument === "--plugins" ||
      argument === "--plugin-dir" ||
  ...
  const hasAmbientPluginInput =
    environment.ACFS_PLUGIN_PATHS !== undefined || environment.ACFS_PLUGINS_DIR !== undefined;
...
async function main(): Promise<void> {
  ...
  if (!basicValidation.valid) {
    console.error("");
    console.error(`Manifest validation failed with ${basicValidation.errors.length} error(s):`);
  ...
  if (!existsSync(CHECKSUMS_PATH)) {
    ...
    console.error(
      "Refusing to generate scripts that require checksum verification without checksums.yaml.",
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const checksumValidationErrors = validateVerifiedInstallerChecksums(manifest, installers);
    ...
    console.error(
      `Failed to parse checksums.yaml: ${err instanceof Error ? err.message : String(err)}`,
    );
    process.exit(1);
  }
  ...
  // Web data: TypeScript modules for apps/web
  {
    ...
    filesToGenerate.set(commandsPath, {
      content: generateWebCommands(effectiveManifest),
      mode: 0o644,
    });
    ...
    filesToGenerate.set(lessonsPath, {
      content: generateWebLessonsIndex(effectiveManifest),
      mode: 0o644,
    });
  ...
  const actualPaths = [OUTPUT_DIR, WEB_OUTPUT_DIR].flatMap((directory) =>
    existsSync(directory) ? readdirSync(directory).map((name) => join(directory, name)) : [],
  );
  ...
  for (const [filepath, region] of regionsToPatch) {
    ...
    if (region.error !== null || region.content === null) {
      throw new Error(
        `Cannot update the generated region in ${filename}: ${region.error ?? "render produced no content"}`,
      );
    }
