...
import {
  parsePluginJsonBytes, readPluginInputFile, readVerifiedPluginArchive,
  type VerifiedPluginArchive,
} from './plugin-archive.js';

export interface PluginArchiveTarget { os: string; version: string; arch: string; libc: string }
...
export class PluginReviewError extends Error {
  constructor(public readonly code: 'plugin_review_required' | 'plugin_target_unsupported', message: string) {
    super(message);
    this.name = 'PluginReviewError';
  }
}
const TARGET_KEYS = ['arch', 'libc', 'os', 'version'];
const REVIEW_KEYS = ['approvedCapabilities', 'expiresAt', 'packageId', 'packageSha256',
  'reviewedAt', 'reviewer', 'schema', 'sourceCommit', 'target', 'version'];
...
function targetValid(value: unknown): value is PluginArchiveTarget {
  return record(value) && Object.keys(value).sort().join(',') === TARGET_KEYS.join(',')
    && TARGET_KEYS.every((key) => typeof value[key] === 'string'
      && /^[a-z0-9][a-z0-9_.-]{0,63}$/.test(value[key] as string));
}
...
function canonicalTime(value: unknown): number {
  ...
  if (value !== iso && value !== iso.replace('.000Z', 'Z')) return refuse('Review timestamp is not canonical');
  return timestamp;
}
...
export function readPluginReviewRecord(reviewPath: string, now = Date.now()): PluginReviewRecord {
  ...
  if (!record(input) || Object.keys(input).sort().join(',') !== REVIEW_KEYS.join(',')
      || input.schema !== 'acfs.plugin-review.v1'
      || typeof input.packageId !== 'string' || !/^[a-z][a-z0-9_.-]{0,127}$/.test(input.packageId)
      || !safeText(input.version, 128) || !safeText(input.reviewer, 200)
      || typeof input.packageSha256 !== 'string' || !/^[a-f0-9]{64}$/i.test(input.packageSha256)
      || typeof input.sourceCommit !== 'string' || !/^[a-f0-9]{40}$/i.test(input.sourceCommit)
      || !targetValid(input.target) || !Array.isArray(input.approvedCapabilities)
      || input.approvedCapabilities.length === 0
      || !input.approvedCapabilities.every((cap) => typeof cap === 'string' && BASELINE_CAPABILITIES.has(cap))
  ...
  return {
    schema: 'acfs.plugin-review.v1', packageId: input.packageId, version: input.version,
    sourceCommit: input.sourceCommit.toLowerCase(), packageSha256: input.packageSha256.toLowerCase(),
    reviewer: input.reviewer, reviewedAt: input.reviewedAt as string, expiresAt: input.expiresAt as string,
    target: { ...input.target }, approvedCapabilities: [...input.approvedCapabilities] as string[],
  };
}
...
export function readReviewedPluginArchive(
  ...
  if (!targetValid(target)) {
    throw new PluginReviewError('plugin_target_unsupported', 'An explicit valid target is required');
  }
  const review = readPluginReviewRecord(reviewPath, now);
  if (!sameTarget(target, review.target)) {
    throw new PluginReviewError('plugin_target_unsupported', 'Requested target differs from the external review');
  }
  const archive = readVerifiedPluginArchive(archivePath, review.packageSha256);
  const manifest = archive.manifest;
  if (!record(manifest) || !record(manifest.provenance)
      || manifest.packageId !== review.packageId || manifest.version !== review.version
  ...
  if (!Array.isArray(manifest.targets) || !manifest.targets.some((candidate) =>
    record(candidate) && candidate.os === target.os
      && Array.isArray(candidate.versions) && candidate.versions.includes(target.version)
      && Array.isArray(candidate.arch) && candidate.arch.includes(target.arch)
      && Array.isArray(candidate.libc) && candidate.libc.includes(target.libc))) {
    throw new PluginReviewError('plugin_target_unsupported', 'Package does not declare the complete requested target tuple');
  }
  if (!record(manifest.capabilities) || !Array.isArray(manifest.capabilities.allowed)
      || !manifest.capabilities.allowed.every((capability) =>
