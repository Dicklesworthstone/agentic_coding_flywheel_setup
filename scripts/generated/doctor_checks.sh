...
declare -a MANIFEST_CHECKS=(
    ...
    "stack.eidetic_engine_cli	Durable, local-first, explainable memory for coding agents (ee)	ee --version || ee --help	optional	target_user"
    "stack.franken_markdown	Pure-Rust Markdown engine rendering self-contained HTML and tagged PDF (fmd)	fmd --version || fmd --help	optional	target_user"
    "stack.pi_agent_rust	Native single-binary Rust port of the Pi coding agent (pi)	pi --version || pi --help	optional	target_user"
    "stack.power_failure_resumer	Recover crashed coding-agent sessions after a hard power cut (pfr)	command -v pfr	optional	target_user"
